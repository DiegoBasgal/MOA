...
from simulador.src.reg import *
from simulador.src.const import *

lock = threading.Lock()
logger = logging.getLogger("__main__")

class Planta:
    def __init__(self, shared_dict, dj52L: Dj52L, ugs: list([Ug])) -> None:
        self.ugs = ugs
        self.dj52L = dj52L
        ...
        while not self.dict.GLB["stop_sim"]:
            ...
            try:
                ...
                for ug in self.ugs:
                    ...
                    elif not self.dict.UG[f"trip_condic_ug{ug.id}"] and self.self.dict.BORDA[f"ug{ug.id}_condic"]:
                        DataBank.set_words(REG[f"REG_UG{ug.id}_Emergencia_Condicionadores"], [0])
                        self.dict.BORDA[f"ug{ug.id}_condic"] = False

                    self.leitura_data_bank_ugs(ug)
                    ug.passo()
                ...
                for ug in self.ugs: self.dict.USN["q_liquida"] -= self.dict.UG[f"q_ug{ug.id}"]
                ...
                lock.release()
                tempo_restante = (self.passo_simulacao - (self.get_time() - t_inicio_passo).seconds)
                if tempo_restante > 0:
    ...
    def q_sanitaria(self) -> float:
        return 0.22

    def leitura_data_bank_usn(self) -> None:
    ...
    def leitura_data_bank_ugs(self, ug: Ug) -> None:
    ...
    def escrita_data_bank(self) -> None:
        try:
            for ug in self.ugs:
                ...
                DataBank.set_words(REG[f"REG_UG{ug.id}_Pressao_CX_Espiral"], [round(10 * self.dict.USN[f"pressao_caixa_espiral_ug{ug.id}"])],)
                DataBank.set_words(REG[f"REG_UG{ug.id}_Temperatura_01"], [round(self.dict[f"temperatura_ug{ug.id}_fase_r"])],)
                DataBank.set_words(REG[f"REG_UG{ug.id}_Temperatura_02"], [round(self.dict[f"temperatura_ug{ug.id}_fase_s"])],)
                DataBank.set_words(REG[f"REG_UG{ug.id}_Temperatura_03"], [round(self.dict[f"temperatura_ug{ug.id}_fase_t"])],)
                DataBank.set_words(REG[f"REG_UG{ug.id}_Temperatura_04"], [round(self.dict[f"temperatura_ug{ug.id}_nucleo_gerador_1"])],)
                DataBank.set_words(REG[f"REG_UG{ug.id}_Temperatura_05"], [round(self.dict[f"temperatura_ug{ug.id}_nucleo_gerador_2"])],)
                DataBank.set_words(REG[f"REG_UG{ug.id}_Temperatura_06"], [round(self.dict[f"temperatura_ug{ug.id}_nucleo_gerador_3"])],)
                DataBank.set_words(REG[f"REG_UG{ug.id}_Temperatura_07"], [round(self.dict[f"temperatura_ug{ug.id}_mancal_casq_rad"])],)
                DataBank.set_words(REG[f"REG_UG{ug.id}_Temperatura_08"], [round(self.dict[f"temperatura_ug{ug.id}_mancal_casq_comb"])],)
                DataBank.set_words(REG[f"REG_UG{ug.id}_Temperatura_09"], [round(self.dict[f"temperatura_ug{ug.id}_mancal_escora_comb"])],)

            DataBank.set_words(REG["REG_USINA_NivelBarragem"], [round((self.dict["nv_montante"]) * 10000)])
            DataBank.set_words(REG["REG_USINA_NivelCanalAducao"], [round((self.dict["nv_jusante_grade"]) * 10000)])  # TODO ?
            DataBank.set_words(REG["REG_USINA_Subestacao_PotenciaAtivaMedia"], [round(self.dict["potencia_kw_se"])])
            DataBank.set_words(REG["REG_USINA_Subestacao_TensaoRS"], [round(self.dict["tensao_na_linha"] / 1000)])
            DataBank.set_words(REG["REG_USINA_Subestacao_TensaoST"], [round(self.dict["tensao_na_linha"] / 1000)])
            DataBank.set_words(REG["REG_USINA_Subestacao_TensaoTR"], [round(self.dict["tensao_na_linha"] / 1000)])
            DataBank.set_words(REG["REG_USINA_potencia_kw_mp"], [round(max(0, self.dict["potencia_kw_mp"]))])
            DataBank.set_words(REG["REG_USINA_potencia_kw_mr"], [round(max(0, self.dict["potencia_kw_mr"]))])
