...
class Ug:
    def __init__(self, id, shared_dict) -> None:
        self.id = id
    ...
    def controle_reservatorio(self) -> None:
        if (self.etapa_atual > ETAPA_UP) and (self.dict.UG["nv_montante"] < USINA_NV_MINIMO_OPERACAO):
            self.potencia = 0
    ...
    def controle_etapas(self) -> None:
        ...
        if self.etapa_atual == ETAPA_UPS:
            ...
            if (self.etapa_alvo is None) or (self.etapa_alvo == self.etapa_atual):
                ...
                if (self.tempo_na_transicao >= TEMPO_TRANS_UPS_US) and self.dict.UG["dj52L_fechado"]:
        ...
        if self.etapa_atual == ETAPA_US:
            if (self.etapa_alvo is None) or (self.etapa_alvo == self.etapa_atual):
                ...
                if self.dict.UG["dj52L_fechado"] and not self.dict.UG["dj52L_trip"]:
