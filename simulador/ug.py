...
class Unidade:
    ...
    def passo(self) -> 'None':
        ...
        if LEI.ler_bit(MB['TDA'][f'CP{self.id}_CMD_FECHAMENTO']) or self.dict['TDA'][f'cp{self.id}_thread_fechada']:
        ...
        if LEI.ler_bit(MB['TDA'][f'CP{self.id}_CMD_ABERTURA_TOTAL']) or self.dict['TDA'][f'cp{self.id}_thread_aberta']:
        ...
        if LEI.ler_bit(MB['TDA'][f'CP{self.id}_CMD_ABERTURA_CRACKING']) or self.dict['TDA'][f'cp{self.id}_thread_cracking']:
    ...
    def controle_limites(self) -> 'None':
    ...
    def controle_etapas(self) -> 'None':
        # Unidade Parada
        if self.etapa_atual == ETAPA_UP:
            self.dict[f'UG{self.id}'][f'potencia'] = self.potencia = 0
        ...
        if self.etapa_atual == ETAPA_US:
            if self.etapa_alvo == self.etapa_atual:
                ...
                if self.dict['SE']['dj_fechado']:
                    self.dict[f'UG{self.id}'][f'potencia'] = self.potencia = min(max(self.potencia, POT_MIN), POT_MAX)
                ...
                if self.dict['SE']['dj_aberto'] or self.dict['SE']['dj_trip']:
                    self.dict[f'UG{self.id}'][f'setpoint'] = self.setpoint = 0
                    self.dict[f'UG{self.id}'][f'etapa_atual'] = self.etapa_atual = ETAPA_UVD
                    self.dict[f'UG{self.id}'][f'etapa_alvo'] = self.etapa_alvo = ETAPA_US
                    self.tempo_transicao = 0
    ...
    def controlar_fechamento_comporta(self) -> 'None':
        ...
        elif self.dict['TDA'][f'cp{self.id}_aberta'] or self.dict['TDA'][f'cp{self.id}_cracking']:
            self.dict['TDA'][f'cp{self.id}_cracking'] = self.dict['TDA'][f'cp{self.id}_aberta'] = False
    ...
    def controlar_abertura_comporta(self) -> 'None':
        if  self.dict['TDA']['lg_operando'] \
            or self.dict['TDA'][f'cp{self.id}_fechada'] \
            or not self.dict['TDA'][f'cp{self.id}_permissao'] \
            or self.dict['TDA'][f'cp{2 if self.id == 1 else 1}_operando']:
            ...
            print(f'[UG{self.id}-CP{self.id}] Comporta {self.id} sem Permissão de Abertura.') if not self.dict['TDA'][f'cp{self.id}_permissao'] else None
            print(f'[UG{self.id}-CP{self.id}] Comporta Fechada. Executar operação de Cracking primeiro!') if self.dict['TDA'][f'cp{self.id}_fechada'] else None
            print(f'[UG{self.id}-CP{self.id}] Comporta {2 if self.id == 1 else 1} em operação.') if self.dict['TDA'][f'cp{2 if self.id == 1 else 1}_operando'] else None
            ...
            self.dict['TDA'][f'cp{self.id}_fechada'] = self.dict['TDA'][f'cp{self.id}_cracking'] = False

            while self.dict['TDA'][f'cp{self.id}_progresso'] <= 100:
                self.dict['TDA'][f'uh_disponivel'] = False
                self.dict['TDA'][f'cp{self.id}_progresso'] += 0.000001

            self.dict['TDA'][f'cp{self.id}_aberta'] = self.dict['TDA'][f'uh_disponivel'] = True
            self.dict['TDA'][f'cp{self.id}_aguardando'] = self.dict['TDA'][f'cp{self.id}_equalizar'] = False
            self.dict['TDA'][f'cp{self.id}_operando'] = False

    def controlar_cracking_comporta(self) -> 'None':
        if  self.dict['TDA']['lg_operando'] \
            or self.dict['TDA'][f'cp{self.id}_aberta'] \
            or self.dict['TDA'][f'cp{2 if self.id == 1 else 1}_operando']:
            print(f'[UG{self.id}-CP{self.id}] Não é possível realizar o Cracking da Comporta.')
            print(f'[UG{self.id}-CP{self.id}] Limpa Grades em operação.') if self.dict['TDA']['lg_operando'] else None
            print(f'[UG{self.id}-CP{self.id}] A Comporta ja está Aberta.') if self.dict['TDA'][f'cp{self.id}_aberta'] else None
            print(f'[UG{self.id}-CP{self.id}] Comporta {2 if self.id == 1 else 1} em operação.') if self.dict['TDA'][f'cp{2 if self.id == 1 else 1}_operando'] else None

        elif self.dict['TDA'][f'cp{self.id}_fechada'] and not self.dict['TDA'][f'cp{self.id}_aberta']:
            self.dict['TDA'][f'cp{self.id}_operando'] = True
            self.dict['TDA'][f'cp{self.id}_aberta'] = self.dict['TDA'][f'cp{self.id}_fechada'] = False

            while self.dict['TDA'][f'cp{self.id}_progresso'] <= 20:
                self.dict['TDA'][f'uh_disponivel'] = False
                self.dict['TDA'][f'cp{self.id}_progresso'] += 0.000001

            self.dict['TDA'][f'cp{self.id}_cracking'] = self.dict['TDA'][f'uh_disponivel'] = True
            self.dict['TDA'][f'cp{self.id}_operando'] = False
    ...
    def equalizar_unidade_hidraulica(self) -> 'None':
        while time() <= time() + 15:
            self.dict['TDA'][f'cp{self.id}_pressao_equalizada'] = self.dict['TDA']['uh_disponivel'] = False
            ...
            if self.dict['TDA'][f'cp{self.id}_trip']:
                ...
                self.dict['TDA']['uh_disponivel'] = self.dict['TDA'][f'cp{self.id}_thread_fechada'] = True
                return

        print(f'[UG{self.id}-CP{self.id}] A Unidade foi Equalizada, Permissão para Abrir comporta ativada!')
        self.dict['TDA'][f'cp{self.id}_pressao_equalizada'] = self.dict['TDA']['uh_disponivel'] = True
        self.dict['TDA'][f'cp{self.id}_aguardando'] = self.dict['TDA'][f'cp{self.id}_permissao'] = True

    def atualizar_modbus(self) -> 'None':
        if self.dict['TDA'][f'cp{self.id}_operando'] and not self.dict['BRD'][f'cp{self.id}_operando']:
            self.dict['BRD'][f'cp{self.id}_operando'] = True
            ESC.escrever_bit(MB['TDA'][f'CP{self.id}_OPERANDO'], valor=1)

        elif not self.dict['TDA'][f'cp{self.id}_operando'] and self.dict['BRD'][f'cp{self.id}_operando']:
            self.dict['BRD'][f'cp{self.id}_borda_o'] = False
        ...
        DB.set_words(MB[f'UG{self.id}']['RV_ESTADO_OPERACAO_2'], [int(self.etapa_alvo)])
