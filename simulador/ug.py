...
class Unidade:
    ...
    def passo(self) -> 'None':
        ...
        self.dict[f'UG{self.id}'][f'setpoint'] = DB.get_words(MB[f'UG{self.id}']['SETPONIT'])[0]
    ...
    def controlar_fechamento_comporta(self) -> 'None':
        if self.dict[f'UG{self.id}'][f'etapa_atual'] != ETAPA_UP:
            ...
            self.dict[f'CP{self.id}'][f'cracking'] = self.dict[f'CP{self.id}'][f'aberta'] = False

            while self.dict[f'CP{self.id}'][f'progresso'] >= 0:
                self.dict[f'CP{self.id}'][f'progresso'] -= 0.000001
    ...
    def controlar_abertura_comporta(self) -> 'None':
        if  self.dict['TDA']['lg_operando'] \
            ...
            self.dict[f'CP{self.id}'][f'fechada'] = self.dict[f'CP{self.id}'][f'cracking'] = False

            while self.dict[f'CP{self.id}'][f'progresso'] <= 100:
                self.dict['TDA'][f'uh_disponivel'] = False
                self.dict[f'CP{self.id}'][f'progresso'] += 0.000001

            self.dict[f'CP{self.id}'][f'aberta'] = self.dict['TDA'][f'uh_disponivel'] = True
            self.dict[f'CP{self.id}'][f'aguardando'] = self.dict[f'CP{self.id}'][f'equalizar'] = False
    ...
    def controlar_cracking_comporta(self) -> 'None':
        if  self.dict['TDA']['lg_operando'] \
            ...
            self.dict[f'CP{self.id}'][f'operando'] = True
            self.dict[f'CP{self.id}'][f'aberta'] = self.dict[f'CP{self.id}'][f'fechada'] = False

            while self.dict[f'CP{self.id}'][f'progresso'] <= 20:
                self.dict['TDA'][f'uh_disponivel'] = False
                self.dict[f'CP{self.id}'][f'progresso'] += 0.000001

            self.dict[f'CP{self.id}'][f'cracking'] = self.dict['TDA'][f'uh_disponivel'] = True
            self.dict[f'CP{self.id}'][f'operando'] = False
    ...
    def equalizar_unidade_hidraulica(self) -> 'None':
        while time() <= time() + 15:
            self.dict[f'CP{self.id}'][f'pressao_equalizada'] = self.dict['TDA']['uh_disponivel'] = False

            if self.dict[f'CP{self.id}'][f'trip']:
                self.dict[f'CP{self.id}'][f'trip'] = False

                print(f'[UG{self.id}-CP{self.id}] Não foi possível Equalizar a Unidade Hidráulica! Fechando Comporta...')
                self.dict['TDA']['uh_disponivel'] = self.dict[f'CP{self.id}'][f'thread_fechada'] = True
                return

        print(f'[UG{self.id}-CP{self.id}] A Unidade foi Equalizada, Permissão para Abrir comporta ativada!')
        self.dict[f'CP{self.id}'][f'pressao_equalizada'] = self.dict['TDA']['uh_disponivel'] = True
        self.dict[f'CP{self.id}'][f'aguardando'] = self.dict[f'CP{self.id}'][f'permissao'] = True
    ...
    def atualizar_modbus(self) -> 'None':
        ...
        if self.dict[f'CP{self.id}']['aguardando'] and not self.dict['BRD'][f'cp{self.id}_aguardando']:
            ...
            ESC.escrever_bit(MB["TDA"][f"CP{self.id}_AGUARDANDO_CMD_ABERTURA"], valor=2)
