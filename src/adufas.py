...
class Adufas:
    ...
    class Comporta:
        def __init__(self, id: "int") -> "None":
            ...
            dct_ad[f'CP{self.id}']['manual'] = lei.LeituraModbusBit(
                self.clp["AD"],
                REG_AD[f"CP_0{self.id}_ACION_LOCAL"],
                descricao=f"[AD][CP{self.id}] Comporta Manual"
            )
            dct_ad[f'CP{self.id}']['posicao'] = lei.LeituraModbus(
                self.clp["AD"],
        ...
        def calcular_setpoint(self) -> "None":
            ...
            logger.debug(f"[AD][CP{self.id}]      Comporta:                  \"{ADCP_STR_DCT_ESTADO[dct_ad[f'CP{self.id}']['estado']]}\"")
            logger.debug(f"[AD][CP{self.id}]      Etapa:                     \"{ADCP_STR_DCT_ETAPA[self.etapa]}\"")
            logger.debug(f"[AD][CP{self.id}]      Leituras:")
            logger.debug(f"[AD][CP{self.id}]      - \"Posição\":               {dct_ad[f'CP{self.id}']['posicao']}")
            logger.debug("")

            if dct_ad[f'CP{self.id}']['manual'] or dct_ad[f'CP{self.id}']['estado'] == ADCP_MANUAL:
                dct_ad[f'CP{self.id}']['estado'] == ADCP_MANUAL
                return

            elif dct_ad[f'CP{self.id}']['estado'] == ADCP_INDISPONIVEL:
                return

            else:
                dct_ad[f'CP{self.id}']['estado'] = ADCP_DISPONIVEL

                if not self.verificar_permissivos():
                    return

                erro = (dct_tda['nivel_montante'].valor - dct_usn['CFG']["ad_nv_alvo"]) * dct_ad[f'CP{self.id}']['k']

                dct_ad[f'CP{self.id}']['controle_p'] = dct_usn['CFG']["ad_kp"] * erro
                dct_ad[f'CP{self.id}']['controle_i'] = min(max(0, dct_usn['CFG']["ad_ki"] * erro + dct_ad[f'CP{self.id}']['controle_i']), 6000)

                sp = min(max(0, dct_ad[f'CP{self.id}']['controle_p'] + dct_ad[f'CP{self.id}']['controle_i']), 6000)

                logger.debug(f"[AD][CP{self.id}]      P:                         {dct_ad[f'CP{self.id}']['controle_p']:1.4f}")
                logger.debug(f"[AD][CP{self.id}]      I:                         dct_ad[f'CP{self.id}']{['controle_i']:1.4f}")
                logger.debug(f"[AD][CP{self.id}]      ERRO:                      {erro}")

                dct_ad[f'CP{self.id}']['setpoint'] = sp

                self.enviar_setpointdct_ad[f'CP{self.id}'](['setpoint'])
    ...
    dct_ad[f'Comporta1'] = Comporta(1)
    dct_ad[f'Comporta2'] = Comporta(2)

    cps = [dct_ad[f'Comporta1'], dct_ad[f'Comporta2']]
    ...
    def verificar_condicionadores(cls) -> "list[c.CondicionadorBase]":
        ...
        if True in (condic.ativo for condic in dct_ad['condicionadores_essenciais']):
            condics_ativos = [condic for condics in [dct_ad['condicionadores_essenciais'], dct_ad['condicionadores']] for condic in condics if condic.ativo]

            logger.debug("")
            if dct_ad['condicionadores_ativos'] == []:
                logger.warning(f"[AD]  Foram detectados Condicionadores ativos no Canal de Adução!")
            else:
                logger.info(f"[AD]  Ainda há Condicionadores ativos no Canal de Adução!")

            for condic in condics_ativos:
                if condic in dct_ad['condicionadores_ativos']:
                    ...
                    dct_ad['condicionadores_ativos'].append(condic)
    ...
    def carregar_leituras(cls) -> "None":
        ...
        dct_ad['condicionadores_essenciais'].append(c.CondicionadorBase(cls.l_alm_28_b_00, CONDIC_NORMALIZAR))

        cls.l_alm_28_b_12 = lei.LeituraModbusBit(cls.clp["AD"], REG_AD["Alarme28_12"], descricao="[AD]  UHCD - Botão de Emergência Pressionado")
        dct_ad['condicionadores_essenciais'].append(c.CondicionadorBase(cls.l_alm_28_b_12, CONDIC_NORMALIZAR))


        cls.l_uhcd_operacional = lei.LeituraModbusBit(cls.clp["AD"], REG_AD["UHCD_OPERACIONAL"], descricao="[AD]  UHCD - Operacional")
        dct_ad['condicionadores'].append(c.CondicionadorBase(cls.l_uhcd_operacional, CONDIC_INDISPONIBILIZAR))

        cls.l_pcad_falta_fase = lei.LeituraModbusBit(cls.clp["AD"], REG_AD["PCAD_FALTA_FASE"], descricao="[AD]  PCAD - Falta Fase")
        dct_ad['condicionadores'].append(c.CondicionadorBase(cls.l_pcad_falta_fase, CONDIC_INDISPONIBILIZAR))

        cls.l_alm_28_b_01 = lei.LeituraModbusBit(cls.clp["AD"], REG_AD["Alarme28_01"], descricao="[AD]  Relé Falta de Fase CA Atuado")
        dct_ad['condicionadores'].append(c.CondicionadorBase(cls.l_alm_28_b_01, CONDIC_NORMALIZAR))

        cls.l_alm_28_b_04 = lei.LeituraModbusBit(cls.clp["AD"], REG_AD["Alarme28_04"], descricao="[AD]  UHCD - Pressão de Óleo Baixa")
        dct_ad['condicionadores'].append(c.CondicionadorBase(cls.l_alm_28_b_04, CONDIC_NORMALIZAR))
        ...
        dct_ad['condicionadores'].append(c.CondicionadorBase(cls.l_alm_28_b_05, CONDIC_NORMALIZAR))
        ...
        dct_ad['condicionadores'].append(c.CondicionadorBase(cls.l_alm_29_b_00, CONDIC_NORMALIZAR))
        ...
        dct_ad['condicionadores'].append(c.CondicionadorBase(cls.l_alm_29_b_01, CONDIC_NORMALIZAR))
        ...
        dct_ad['condicionadores'].append(c.CondicionadorBase(cls.l_alm_29_b_02, CONDIC_NORMALIZAR))
        ...
        dct_ad['condicionadores'].append(c.CondicionadorBase(cls.l_alm_29_b_03, CONDIC_NORMALIZAR))

        cls.l_alm_29_b_05 = lei.LeituraModbusBit(cls.clp["AD"], REG_AD["Alarme29_05"], descricao="[AD]  Comporta 01 - Falha na Abertura")
        dct_ad['condicionadores'].append(c.CondicionadorBase(cls.l_alm_29_b_05, CONDIC_NORMALIZAR))

        cls.l_alm_29_b_06 = lei.LeituraModbusBit(cls.clp["AD"], REG_AD["Alarme29_06"], descricao="[AD]  Comporta 01 - Falha no Fechamento")
        dct_ad['condicionadores'].append(c.CondicionadorBase(cls.l_alm_29_b_06, CONDIC_NORMALIZAR))
        ...
        dct_ad['condicionadores'].append(c.CondicionadorBase(cls.l_alm_29_b_07, CONDIC_NORMALIZAR))

        cls.l_alm_29_b_09 = lei.LeituraModbusBit(cls.clp["AD"], REG_AD["Alarme29_09"], descricao="[AD]  Comporta 02 - Falha na Abertura")
        dct_ad['condicionadores'].append(c.CondicionadorBase(cls.l_alm_29_b_09, CONDIC_NORMALIZAR))

        cls.l_alm_29_b_10 = lei.LeituraModbusBit(cls.clp["AD"], REG_AD["Alarme29_10"], descricao="[AD]  Comporta 02 - Falha no Fechamento")
        dct_ad['condicionadores'].append(c.CondicionadorBase(cls.l_alm_29_b_10, CONDIC_NORMALIZAR))
        ...
        dct_ad['condicionadores'].append(c.CondicionadorBase(cls.l_alm_30_b_08, CONDIC_NORMALIZAR))
        ...
        dct_ad['condicionadores'].append(c.CondicionadorBase(cls.l_alm_30_b_09, CONDIC_NORMALIZAR))
        ...
        dct_ad['condicionadores'].append(c.CondicionadorBase(cls.l_alm_30_b_10, CONDIC_NORMALIZAR))
        ...
        dct_ad['condicionadores'].append(c.CondicionadorBase(cls.l_alm_30_b_11, CONDIC_NORMALIZAR))
        ...
        dct_ad['condicionadores'].append(c.CondicionadorBase(cls.l_alm_31_b_00, CONDIC_NORMALIZAR))
        ...
        dct_ad['condicionadores'].append(c.CondicionadorBase(cls.l_alm_31_b_01, CONDIC_NORMALIZAR))
        ...
        dct_ad['condicionadores'].append(c.CondicionadorBase(cls.l_alm_31_b_02, CONDIC_NORMALIZAR))
        ...
        dct_ad['condicionadores'].append(c.CondicionadorBase(cls.l_alm_31_b_03, CONDIC_NORMALIZAR))
        ...
        dct_ad['condicionadores'].append(c.CondicionadorBase(cls.l_alm_31_b_04, CONDIC_NORMALIZAR))
        ...
        dct_ad['condicionadores'].append(c.CondicionadorBase(cls.l_alm_31_b_05, CONDIC_NORMALIZAR))
        ...
        dct_ad['condicionadores'].append(c.CondicionadorBase(cls.l_alm_31_b_06, CONDIC_NORMALIZAR))
