...
class Usina:
    ...
    def distribuir_potencia(self, pot_alvo):
        ...
        if self.pot_medidor > self.pot_maxima_alvo and pot_alvo > (self.pot_maxima_alvo * 0.95):
            pot_alvo = pot_alvo * 0.99 * (self.pot_maxima_alvo / self.pot_medidor)

        if pot_alvo < self.pot_minima:
            pot_alvo = 0
            self.ug1.mudar_setpoint(0)
            self.ug2.mudar_setpoint(0)
        else:
            pot_alvo = min(pot_alvo, self.pot_disp)
            if self.ug1.sincronizada and self.ug2.sincronizada and pot_alvo > (2 * self.pot_minima):
                self.ug1.mudar_setpoint(pot_alvo / 2)
                self.ug2.mudar_setpoint(pot_alvo / 2)
            elif (pot_alvo > (self.pot_maxima_ug + self.margem_pot_critica)) and (abs(self.erro_nv) > 0.05) \
                    and self.ug1.disponivel and self.ug2.disponivel:
                self.ug1.mudar_setpoint(pot_alvo / 2)
                self.ug2.mudar_setpoint(pot_alvo / 2)
            else:
                pot_alvo = min(pot_alvo, self.pot_maxima_ug)
    ...
    def ler_valores(self):
        ...
        self.nv_montante_recente = sum(self.nv_montante_recentes) / self.n_movel_R
        self.nv_montante_anteriores.append(self.nv_montante)
        self.nv_montante_anteriores = self.nv_montante_anteriores[1:]
        self.nv_montante_anterior = sum(self.nv_montante_anteriores) / self.n_movel_L
