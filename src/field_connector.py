...
class FieldConnector:
    ...
    def get_flag_ug1(self):
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme01 - 1)[0] & (2**0) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG01.Alarmes.Alarme01.Bit00 01.00 - Emergência Supervisório Pressionada")
       ...
       # PCH_Covo.Driver.UG01.Alarmes.Alarme01.Bit01 01.01 - PCP-U1 - Botão de Emergência Pressionado
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme01 - 1)[0] & (2**1) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG01.Alarmes.Alarme01.Bit01 01.01 - PCP-U1 - Botão de Emergência Pressionado")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme01 - 1)[0] & (2**2) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG01.Alarmes.Alarme01.Bit02 01.02 - Q49 - Botão de Emergência Pressionado")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme04 - 1)[0] & (2**2) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG01.Alarmes.Alarme04.Bit02 04.02 - Reg Velocidade - TRIP")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme04 - 1)[0] & (2**5) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG01.Alarmes.Alarme04.Bit05 04.05 - Dispositivo de SobreVelocidade Mecânico Atuado")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme04 - 1)[0] & (2**12) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG01.Alarmes.Alarme04.Bit12 04.12 - Reg Tensão - TRIP")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme05 - 1)[0] & (2**6) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG01.Alarmes.Alarme05.Bit06 05.06 - Relé de Bloqueio 86M Trip Atuado")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme05 - 1)[0] & (2**7) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG01.Alarmes.Alarme05.Bit07 05.07 - Relé de Bloqueio 86M Trip Atuado Temporizado")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme05 - 1)[0] & (2**8) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG01.Alarmes.Alarme05.Bit08 05.08 - Relé de Bloqueio 86M Trip Atuado pelo CLP")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme05 - 1)[0] & (2**9) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG01.Alarmes.Alarme05.Bit09 05.09 - Relé de Bloqueio 86E Trip Atuado")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme05 - 1)[0] & (2**10) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG01.Alarmes.Alarme05.Bit10 05.10 - Relé de Bloqueio 86E Trip Atuado Temporizado")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme05 - 1)[0] & (2**11) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG01.Alarmes.Alarme05.Bit11 05.11 - Relé de Bloqueio 86E Trip Atuado pelo CLP")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme05 - 1)[0] & (2**12) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG01.Alarmes.Alarme05.Bit12 05.12 - Relé de Bloqueio 86H Trip Atuado")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme05 - 1)[0] & (2**13) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG01.Alarmes.Alarme05.Bit13 05.13 - Relé de Bloqueio 86H Trip Atuado pelo CLP")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme06 - 1)[0] & (2**0) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG01.Alarmes.Alarme06.Bit00 06.00 - Relé de Proteção do Gerador - Trip Atuado")
    ...
    def get_ug1_parada(self):
        return True if self.ug1_clp.read_holding_registers(REG_UG1_Turb_Info - 1) & (2**4) == 1 else False
    ...
    def get_flag_ug2(self):
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme01 - 1)[0] & (2**0) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG02.Alarmes.Alarme01.Bit00 01.00 - Emergência Supervisório Pressionada")
       ...
       # PCH_Covo.Driver.UG02.Alarmes.Alarme01.Bit01 01.01 - PCP-U1 - Botão de Emergência Pressionado
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme01 - 1)[0] & (2**1) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG02.Alarmes.Alarme01.Bit01 01.01 - PCP-U1 - Botão de Emergência Pressionado")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme01 - 1)[0] & (2**2) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG02.Alarmes.Alarme01.Bit02 01.02 - Q49 - Botão de Emergência Pressionado")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme04 - 1)[0] & (2**2) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG02.Alarmes.Alarme04.Bit02 04.02 - Reg Velocidade - TRIP")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme04 - 1)[0] & (2**5) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG02.Alarmes.Alarme04.Bit05 04.05 - Dispositivo de SobreVelocidade Mecânico Atuado")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme04 - 1)[0] & (2**12) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG02.Alarmes.Alarme04.Bit12 04.12 - Reg Tensão - TRIP")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme05 - 1)[0] & (2**6) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG02.Alarmes.Alarme05.Bit06 05.06 - Relé de Bloqueio 86M Trip Atuado")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme05 - 1)[0] & (2**7) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG02.Alarmes.Alarme05.Bit07 05.07 - Relé de Bloqueio 86M Trip Atuado Temporizado")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme05 - 1)[0] & (2**8) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG02.Alarmes.Alarme05.Bit08 05.08 - Relé de Bloqueio 86M Trip Atuado pelo CLP")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme05 - 1)[0] & (2**9) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG02.Alarmes.Alarme05.Bit09 05.09 - Relé de Bloqueio 86E Trip Atuado")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme05 - 1)[0] & (2**10) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG02.Alarmes.Alarme05.Bit10 05.10 - Relé de Bloqueio 86E Trip Atuado Temporizado")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme05 - 1)[0] & (2**11) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG02.Alarmes.Alarme05.Bit11 05.11 - Relé de Bloqueio 86E Trip Atuado pelo CLP")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme05 - 1)[0] & (2**12) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG02.Alarmes.Alarme05.Bit12 05.12 - Relé de Bloqueio 86H Trip Atuado")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme05 - 1)[0] & (2**13) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG02.Alarmes.Alarme05.Bit13 05.13 - Relé de Bloqueio 86H Trip Atuado pelo CLP")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme06 - 1)[0] & (2**0) == 0:
            all_low = False
            print("PCH_Covo.Driver.UG02.Alarmes.Alarme06.Bit00 06.00 - Relé de Proteção do Gerador - Trip Atuado")
    ...
    def get_tensao_na_linha(self):
        sn = self.usn_clp.read_holding_registers(REG_USINA_Subestacao_TensaoSN - 1)[0]
        tn = self.usn_clp.read_holding_registers(REG_USINA_Subestacao_TensaoTN - 1)[0]
        rn = self.usn_clp.read_holding_registers(REG_USINA_Subestacao_TensaoRN - 1)[0]
        st = self.usn_clp.read_holding_registers(REG_USINA_Subestacao_TensaoST - 1)[0]
        tr = self.usn_clp.read_holding_registers(REG_USINA_Subestacao_TensaoTR - 1)[0]
        rs = self.usn_clp.read_holding_registers(REG_USINA_Subestacao_TensaoRS - 1)[0]
        return 1

    def partir_ug1(self):
        if not self.get_sincro_ug1():
            response = self.ug1_clp.write_single_register(REG_UG1_Operacao_US - 1, 1)
            print("REG_UG1_Operacao_US(1): ", response)        

    def parar_ug1(self):      
        response = self.ug1_clp.write_single_register(REG_UG1_Operacao_UP - 1, 1)
        print("REG_UG1_Operacao_UP", response)        

    def partir_ug2(self):
        if not self.get_sincro_ug2():
            response = self.ug2_clp.write_single_register(REG_UG2_Operacao_US - 1, 1)
            print("REG_UG2_Operacao_US(1): ", response)    

    def parar_ug2(self):
        response = self.ug2_clp.write_single_register(REG_UG2_Operacao_UP - 1, 1)
        print("REG_UG2_Operacao_UP", response)        
    ...
    def set_ug1_setpoint(self, setpoint):
        ...
        print("REG_UG1_CtrlPotencia_Alvo(" , setpoint, ") response: ", response)
    ...
    def set_ug2_setpoint(self, setpoint):
        ...
        print("REG_UG2_CtrlPotencia_Alvo(" , setpoint, ") response: ", response)
    ...
    def normalizar_emergencia(self):
        print("NORMALIZANDO CLPS RECONHECE E RESET")
        self.ug1_clp.write_single_register(REG_UG1_Operacao_EmergenciaLigar - 1, 0)
        self.ug2_clp.write_single_register(REG_UG2_Operacao_EmergenciaLigar - 1, 0)
        self.usn_clp.write_single_register(REG_USINA_EmergenciaLigar - 1, 0)
        ...
        print("REG_USINA_Disj52LFechar", response)  