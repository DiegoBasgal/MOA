...
class FieldConnector:
    ...
    def get_flag_ug1(self):
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme01 - 1)[0] & (2**0) == 0:
            ...
            if not self.warned_ug1:
                logger.warn("PCH_Covo.Driver.UG01.Alarmes.Alarme01.Bit00 01.00 - Emergência Supervisório Pressionada")
       ...
       # PCH_Covo.Driver.UG01.Alarmes.Alarme01.Bit01 01.01 - PCP-U1 - Botão de Emergência Pressionado
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme01 - 1)[0] & (2**1) == 0:
            all_low = False
            if not self.warned_ug1:
                logger.warn("PCH_Covo.Driver.UG01.Alarmes.Alarme01.Bit01 01.01 - PCP-U1 - Botão de Emergência Pressionado")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme01 - 1)[0] & (2**2) == 0:
            all_low = False
            if not self.warned_ug1:
                logger.warn("PCH_Covo.Driver.UG01.Alarmes.Alarme01.Bit02 01.02 - Q49 - Botão de Emergência Pressionado")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme04 - 1)[0] & (2**2) == 0:
            all_low = False
            if not self.warned_ug1:
                logger.warn("PCH_Covo.Driver.UG01.Alarmes.Alarme04.Bit02 04.02 - Reg Velocidade - TRIP")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme04 - 1)[0] & (2**5) == 0:
            all_low = False
            if not self.warned_ug1:
                logger.warn("PCH_Covo.Driver.UG01.Alarmes.Alarme04.Bit05 04.05 - Dispositivo de SobreVelocidade Mecânico Atuado")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme04 - 1)[0] & (2**12) == 0:
            all_low = False
            if not self.warned_ug1:
                logger.warn("PCH_Covo.Driver.UG01.Alarmes.Alarme04.Bit12 04.12 - Reg Tensão - TRIP")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme05 - 1)[0] & (2**6) == 0:
            all_low = False
            if not self.warned_ug1:
                logger.warn("PCH_Covo.Driver.UG01.Alarmes.Alarme05.Bit06 05.06 - Relé de Bloqueio 86M Trip Atuado")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme05 - 1)[0] & (2**7) == 0:
            all_low = False
            if not self.warned_ug1:
                logger.warn("PCH_Covo.Driver.UG01.Alarmes.Alarme05.Bit07 05.07 - Relé de Bloqueio 86M Trip Atuado Temporizado")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme05 - 1)[0] & (2**8) == 0:
            all_low = False
            if not self.warned_ug1:
                logger.warn("PCH_Covo.Driver.UG01.Alarmes.Alarme05.Bit08 05.08 - Relé de Bloqueio 86M Trip Atuado pelo CLP")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme05 - 1)[0] & (2**9) == 0:
            all_low = False
            if not self.warned_ug1:
                logger.warn("PCH_Covo.Driver.UG01.Alarmes.Alarme05.Bit09 05.09 - Relé de Bloqueio 86E Trip Atuado")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme05 - 1)[0] & (2**10) == 0:
            all_low = False
            if not self.warned_ug1:
                logger.warn("PCH_Covo.Driver.UG01.Alarmes.Alarme05.Bit10 05.10 - Relé de Bloqueio 86E Trip Atuado Temporizado")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme05 - 1)[0] & (2**11) == 0:
            all_low = False
            if not self.warned_ug1:
                logger.warn("PCH_Covo.Driver.UG01.Alarmes.Alarme05.Bit11 05.11 - Relé de Bloqueio 86E Trip Atuado pelo CLP")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme05 - 1)[0] & (2**12) == 0:
            all_low = False
            if not self.warned_ug1:
                logger.warn("PCH_Covo.Driver.UG01.Alarmes.Alarme05.Bit12 05.12 - Relé de Bloqueio 86H Trip Atuado")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme05 - 1)[0] & (2**13) == 0:
            all_low = False
            if not self.warned_ug1:
                logger.warn("PCH_Covo.Driver.UG01.Alarmes.Alarme05.Bit13 05.13 - Relé de Bloqueio 86H Trip Atuado pelo CLP")
        ...
        if not self.ug1_clp.read_holding_registers(REG_UG1_Alarme06 - 1)[0] & (2**0) == 0:
            all_low = False
            if not self.warned_ug1:
                logger.warn("PCH_Covo.Driver.UG01.Alarmes.Alarme06.Bit00 06.00 - Relé de Proteção do Gerador - Trip Atuado")
    ...
    def get_ug1_parada(self):
        return True if self.ug1_clp.read_holding_registers(REG_UG1_Turb_Info - 1)[0] & (2**4) == 1 else False
    ...
    def get_flag_ug2(self):
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme01 - 1)[0] & (2**0) == 0:
            ...
            if not self.warned_ug2:
                        logger.warn("PCH_Covo.Driver.UG02.Alarmes.Alarme01.Bit00 01.00 - Emergência Supervisório Pressionada")
       ...
       # PCH_Covo.Driver.UG02.Alarmes.Alarme01.Bit01 01.01 - PCP-U1 - Botão de Emergência Pressionado
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme01 - 1)[0] & (2**1) == 0:
            all_low = False
            if not self.warned_ug2:
                        logger.warn("PCH_Covo.Driver.UG02.Alarmes.Alarme01.Bit01 01.01 - PCP-U1 - Botão de Emergência Pressionado")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme01 - 1)[0] & (2**2) == 0:
            all_low = False
            if not self.warned_ug2:
                        logger.warn("PCH_Covo.Driver.UG02.Alarmes.Alarme01.Bit02 01.02 - Q49 - Botão de Emergência Pressionado")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme04 - 1)[0] & (2**2) == 0:
            all_low = False
            if not self.warned_ug2:
                        logger.warn("PCH_Covo.Driver.UG02.Alarmes.Alarme04.Bit02 04.02 - Reg Velocidade - TRIP")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme04 - 1)[0] & (2**5) == 0:
            all_low = False
            if not self.warned_ug2:
                        logger.warn("PCH_Covo.Driver.UG02.Alarmes.Alarme04.Bit05 04.05 - Dispositivo de SobreVelocidade Mecânico Atuado")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme04 - 1)[0] & (2**12) == 0:
            all_low = False
            if not self.warned_ug2:
                        logger.warn("PCH_Covo.Driver.UG02.Alarmes.Alarme04.Bit12 04.12 - Reg Tensão - TRIP")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme05 - 1)[0] & (2**6) == 0:
            all_low = False
            if not self.warned_ug2:
                        logger.warn("PCH_Covo.Driver.UG02.Alarmes.Alarme05.Bit06 05.06 - Relé de Bloqueio 86M Trip Atuado")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme05 - 1)[0] & (2**7) == 0:
            all_low = False
            if not self.warned_ug2:
                        logger.warn("PCH_Covo.Driver.UG02.Alarmes.Alarme05.Bit07 05.07 - Relé de Bloqueio 86M Trip Atuado Temporizado")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme05 - 1)[0] & (2**8) == 0:
            all_low = False
            if not self.warned_ug2:
                        logger.warn("PCH_Covo.Driver.UG02.Alarmes.Alarme05.Bit08 05.08 - Relé de Bloqueio 86M Trip Atuado pelo CLP")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme05 - 1)[0] & (2**9) == 0:
            all_low = False
            if not self.warned_ug2:
                        logger.warn("PCH_Covo.Driver.UG02.Alarmes.Alarme05.Bit09 05.09 - Relé de Bloqueio 86E Trip Atuado")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme05 - 1)[0] & (2**10) == 0:
            all_low = False
            if not self.warned_ug2:
                        logger.warn("PCH_Covo.Driver.UG02.Alarmes.Alarme05.Bit10 05.10 - Relé de Bloqueio 86E Trip Atuado Temporizado")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme05 - 1)[0] & (2**11) == 0:
            all_low = False
            if not self.warned_ug2:
                        logger.warn("PCH_Covo.Driver.UG02.Alarmes.Alarme05.Bit11 05.11 - Relé de Bloqueio 86E Trip Atuado pelo CLP")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme05 - 1)[0] & (2**12) == 0:
            all_low = False
            if not self.warned_ug2:
                        logger.warn("PCH_Covo.Driver.UG02.Alarmes.Alarme05.Bit12 05.12 - Relé de Bloqueio 86H Trip Atuado")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme05 - 1)[0] & (2**13) == 0:
            all_low = False
            if not self.warned_ug2:
                        logger.warn("PCH_Covo.Driver.UG02.Alarmes.Alarme05.Bit13 05.13 - Relé de Bloqueio 86H Trip Atuado pelo CLP")
        ...
        if not self.ug2_clp.read_holding_registers(REG_UG2_Alarme06 - 1)[0] & (2**0) == 0:
            all_low = False
            if not self.warned_ug2:
                        logger.warn("PCH_Covo.Driver.UG02.Alarmes.Alarme06.Bit00 06.00 - Relé de Proteção do Gerador - Trip Atuado")
    ...
    def acionar_emergencia(self):
        logger.warn("Acionando emergencia USINA")
    ...
    def acionar_emergencia_ug1(self):
        logger.warn("Acionando emergencia UG1")
    ...
    def acionar_emergencia_ug2(self):
        logger.warn("Acionando emergencia UG2")        
    ...
    def normalizar_emergencia(self):
        ...
        logger.info("Fecha Dj52L {}".format(response))  
