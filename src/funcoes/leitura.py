    ...
    def raw(self) -> int:
        ...
        try:
            if self.__cliente.open():
                if self.__op == 3:
                    aux = self.__cliente.read_holding_registers(
                        self.__registrador
                    )[0]
                elif self.__op == 4:
                    aux = self.__cliente.read_input_registers(self.__registrador)[
                        0
                    ]
                if aux is not None:
                    return aux
...
class LeituraModbusCoil(LeituraBase):
    ...
    def raw(self) -> int:
        ...
        try:
            if self.__cliente.open():
                aux = self.__cliente.read_discrete_inputs(self.__registrador)[0]
                if aux is not None:
                    return aux
                else:
                    return 0
...
class LeituraModbusBit(LeituraModbus):
    def __init__(
        ...
        super().__init__(descr, cliente, registrador)
