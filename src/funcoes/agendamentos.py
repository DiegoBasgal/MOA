...
class Agendamentos:
    ...
    # ATRIBUIÇÂO DE VARIÁVEIS
    cfg: "dict" = None
    usn: "u.Usina" = None

    segundos_passados: "int" = 0
    segundos_adiantados: "int" = 0
    ...
    def verificar_agendamentos_sem_efeito(cls, agendamento) -> "None":
        ...
        if cls.usn.modo_autonomo and not bd.BancoDados.get_executabilidade(agendamento[3])["executavel_em_automatico"]:
        ...
        if not cls.usn.modo_autonomo and not bd.BancoDados.get_executabilidade(agendamento[3])["executavel_em_manual"]:
    ...
    def verificar_agendamentos_usina(cls, agendamento) -> "None":
        ...
        if agendamento[3] == AGN_INDISPONIBILIZAR:
            ...
            while (not cls.usn.ug1.etapa == UG_PARADA and not cls.usn.ug2.etapa == UG_PARADA):
                cls.usn.ler_valores()
        ...
        if agendamento[3] == AGN_ALTERAR_POT_LIMITE_TODAS_AS_UGS:
            try:
                ...
                cls.cfg["pot_maxima_alvo"] = novo

                for ug in cls.usn.ugs:
                    cls.cfg[f"pot_maxima_ug{ug.id}"] = novo / 2
    ...
    def verificar_agendamentos_ugs(cls, agendamento) -> "None":
        ...
        if agendamento[3] == AGN_UG2_ALTERAR_POT_LIMITE:
            try:
                novo = float(agendamento[5].replace(",", "."))
                cls.cfg[f"pot_maxima_ug2"] = novo

            except Exception:
                logger.error(f"[AGN] Valor inválido no agendamento: {agendamento[0]} ({agendamento[5]} é inválido)")

        if agendamento[3] == AGN_UG2_FORCAR_ESTADO_MANUAL:
            cls.usn.ug2.forcar_estado_manual()

        if agendamento[3] == AGN_UG2_FORCAR_ESTADO_DISPONIVEL:
            cls.usn.ug2.forcar_estado_disponivel()

        if agendamento[3] == AGN_UG2_FORCAR_ESTADO_INDISPONIVEL:
            cls.usn.ug2.forcar_estado_indisponivel()

        if agendamento[3] == AGN_UG2_FORCAR_ESTADO_RESTRITO:
            cls.usn.ug2.forcar_estado_restrito()

        if agendamento[3] == AGN_UG2_TEMPO_ESPERA_RESTRITO:
            try:
                cls.usn.ug2.normalizacao_agendada = True
                novo = agendamento[5].split(":")
                tempo = (int(novo[0]) * 3600) + (int(novo[1]) * 60)
                cls.usn.ug2.tempo_normalizar = tempo
