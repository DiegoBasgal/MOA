...
class UnidadeDeGeracao:
    ...
    def etapa_atual(self) -> int:
        try:
            ...
            self.logger.error(f"[UG{self.id}] Não foi possível realizar a leitura da etapa atual.\nException: \"{traceback.print_stack}\"")
            return False
    ...
    def etapa_alvo(self) -> int:
        try:
            ...
            self.logger.error(f"[UG{self.id}] Não foi possível realizar a leitura da etapa alvo.\nException: \"{traceback.print_stack}\"")
            return False
    ...
    def step(self) -> None:
        try:
            ...
            self.logger.error(f"[UG{self.id}] Erro na execução da máquina de estados -> step.\nException: {traceback.format_exc()}")
            raise e
    ...
    def interstep(self) -> None:
        try:
            ...
            self.logger.error(f"[UG{self.id}] Erro na execução da máquina de estados -> interstep.\nException: {traceback.format_exc()}")
            raise e
    ...
    def modbus_update_state_register(self) -> None:
        try:
            ...
            self.logger.error(f"[UG{self.id}] Não foi possível escrever os valores no CLP MOA.\nException: {traceback.format_exc()}")
            raise e
    ...
    def carregar_parametros(self, parametros: dict) -> None:
        try:
            ...
            self.logger.error(f"[UG{self.id}] Não foi possível carregar os parâmetros.\nException: {traceback.format_exc()}")
            raise e

    def partir(self) -> bool:
        try:
            if self.clp_sa.read_coils(SA[f"REG_SA_EntradasDigitais_MXI_SA_QCAP_Disj52A{self.id}Fechado"])[0] != 0:
                self.logger.info(f"[UG{self.id}] O Disjuntor 52A{self.id} está aberto. Favor fechá-lo para partir a UG.")
                return True

            if not self.etapa_atual == UNIDADE_SINCRONIZADA:
                self.logger.info(f"[UG{self.id}] Enviando comando de partida.")
                ...
                response = self.clp_ug.write_single_register(UG[f"REG_UG{self.id}_ComandosDigitais_MXW_IniciaPartida"], 1)
                self.enviar_setpoint(self.setpoint)
            else:
                self.logger.debug(f"[UG{self.id}] A UG já está sincronizada.")
                response = self.clp_ug.write_single_coil(UG[f"REG_UG{self.id}_ComandosDigitais_MXW_Cala_Sirene"], 1)
        except:
            self.logger.error(f"[UG{self.id}] Não foi possível partir a UG.\nException: \"{traceback.print_stack}\"")
            return False
        else:
            return response
    ...
    def parar(self) -> bool:
        try:
            if not self.etapa_atual == UNIDADE_PARADA:
                ...
                response = self.clp_ug.write_single_coil(UG[f"REG_UG{self.id}_ComandosDigitais_MXW_Cala_Sirene"], 1)
                response = self.clp_ug.write_single_register(UG[f"REG_UG{self.id}_ComandosDigitais_MXW_IniciaParada"], 1)
                self.enviar_setpoint(0)
            else:
                self.logger.debug(f"[UG{self.id}] A unidade já está parada.")
    ...
    def enviar_setpoint(self, setpoint_kw: int) -> bool:
        try:
            self.logger.debug(f"[UG{self.id}] Enviando setpoint {int(setpoint_kw)}kW.")
            ...
            self.logger.error(f"[UG{self.id}] Não foi possivel enviar o setpoint.\nException: {traceback.format_exc()}")
            return False
        else:
            return response
    ...
    def acionar_trip_logico(self) -> bool:
        try:
            self.logger.debug(f"[UG{self.id}] Acionando TRIP -> Lógico.")
            response = self.clp_ug.write_single_coil(UG[f"REG_UG{self.id}_ComandosDigitais_MXW_EmergenciaViaSuper"], 1)
        except:
            self.logger.error(f"[UG{self.id}] Não foi possivel acionar o TRIP lógico.\nException: {traceback.format_exc()}")
            return False
        else:
            return response
    ...
    def remover_trip_logico(self) -> bool:
        try:
            self.logger.debug(f"[UG{self.id}] Removendo TRIP -> Lógico.")
            response = self.clp_ug.write_single_coil(UG[f"REG_UG{self.id}_ComandosDigitais_MXW_ResetGeral"], 1)
            ...
            self.logger.error(f"[UG{self.id}] Não foi possível remover o TRIP lógico.\nException: \"{traceback.print_stack}\"")
            return False
        else:
            return response
    ...
    def acionar_trip_eletrico(self) -> bool:
        try:
            ...
            self.logger.debug(f"[UG{self.id}] Acionando TRIP -> Elétrico.")
            self.clp_moa.write_single_coil(self.cfg[f"REG_MOA_OUT_BLOCK_UG{self.id}"], [1])
        except:
            self.logger.error(f"[UG{self.id}] Não foi possível acionar o TRIP elétrico.\nException: \"{traceback.print_stack}\"")
            return False
        else:
            return True
    ...
    def remover_trip_eletrico(self) -> bool:
        try:
            if self.clp_sa.read_coils(SA["REG_SA_ComandosDigitais_MXW_Liga_DJ1"])[0] == 0:
                self.logger.debug(f"[UG{self.id}] Comando recebido -> Fechando DJ52L.")
                self.con.fechaDj52L()
        except Exception:
            self.logger.error(f"[UG{self.id}] Não foi possível fechar o DJ52L.\nException: \"{traceback.print_stack}\"")

        try:
            self.enviar_trip_eletrico = False
            self.logger.debug(f"[UG{self.id}] Removendo TRIP -> Elétrico.")
            ...
            self.logger.error(f"[UG{self.id}] Não foi possível remover o TRIP elétrico.\nException: \"{traceback.print_stack}\"")
            return False
        else:
            return response
    ...
    def reconhece_reset_alarmes(self) -> bool:
        try:
            self.logger.info(f"[UG{self.id}] Enviando comando de reconhecer e resetar alarmes.")
            for x in range(3):
                self.logger.debug(f"[UG{self.id}] Tentativa: {x}/3")
            ...
            self.logger.error(f"[UG{self.id}] Não foi possivel enviar o comando de reconhecer e resetar alarmes.\nException: {traceback.format_exc()}")
            return False
        else:
            return True
    ...
    def forcar_estado_manual(self) -> bool:
        try:
            ...
            self.logger.error(f"[UG{self.id}] Não foi possivel forçar o estado manual.\nException: {traceback.print_stack}")
            return False
        else:
            return True
    ...
    def forcar_estado_restrito(self) -> bool:
        try:
            ...
            self.logger.error(f"[UG{self.id}] Não foi possivel forçar o estado restrito.\nException: {traceback.print_stack}")
            return False
        else:
            return True
    ...
    def forcar_estado_indisponivel(self) -> bool:
        try:
            ...
            self.logger.error(f"[UG{self.id}] Não foi possivel forçar o estado indisponível.\nException: {traceback.print_stack}")
            return False
        else:
            return True
    ...
    def forcar_estado_disponivel(self) -> bool:
        try:
            ...
            self.logger.error(f"[UG{self.id}] Não foi possivel forçar o estado disponível.\nException: {traceback.print_stack}")
            return False
        else:
            return True
    ...
    def controle_cx_espiral(self) -> None:
        ...
        self.logger.debug("[UG{}] PI: {:0.3f} <-- P:{:0.3f} + I:{:0.3f}; ERRO={}".format(self.id, saida_pi, self.cx_controle_p, self.cx_controle_i, erro_press_cx))
        ...
        try:
            ...
            self.logger.error(f"[UG{self.id}] Não foi possivel inserir dados no Banco.\nException: {traceback.print_stack}")
    ...
    def controle_limites_operacao(self) -> None:
        ...
        if fase_r[0] >= fase_r[1]:
            self.logger.warning(f"[UG{self.id}] A temperatura de Fase R da UG passou do valor base! ({fase_r[1]}C) | Leitura: {fase_r[0]}C")
        if fase_r[0] >= 0.9*(fase_r[2] - fase_r[1]) + fase_r[1]:
            self.logger.critical(f"[UG{self.id}] A temperatura de Fase R da UG está muito próxima do limite! ({fase_r[2]}C) | Leitura: {fase_r[0]}C")

        if fase_s[0] >= fase_s[1]:
            self.logger.warning(f"[UG{self.id}] A temperatura de Fase S da UG passou do valor base! ({fase_s[1]}C) | Leitura: {fase_s[0]}C")
        if fase_s[0] >= 0.9*(fase_s[2] - fase_s[1]) + fase_s[1]:
            self.logger.critical(f"[UG{self.id}] A temperatura de Fase S da UG está muito próxima do limite! ({fase_s[2]}C) | Leitura: {fase_s[0]}C")

        if fase_t[0] >= fase_t[1]:
            self.logger.warning(f"[UG{self.id}] A temperatura de Fase T da UG passou do valor base! ({fase_t[1]}C) | Leitura: {fase_t[0]}C")
        if fase_t[0] >= 0.9*(fase_t[2] - fase_t[1]) + fase_t[1]:
            self.logger.critical(f"[UG{self.id}] A temperatura de Fase T da UG está muito próxima do limite! ({fase_t[2]}C) | Leitura: {fase_t[0]}C")

        if nucleo_1[0] >= nucleo_1[1]:
            self.logger.warning(f"[UG{self.id}] A temperatura do Núcleo Gerador 1 da UG passou do valor base! ({nucleo_1[1]}C) | Leitura: {nucleo_1[0]}C")
        if nucleo_1[0] >= 0.9*(nucleo_1[2] - nucleo_1[1]) + nucleo_1[1]:
            self.logger.critical(f"[UG{self.id}] A temperatura do Núcleo Gerador 1 da UG está muito próxima do limite! ({nucleo_1[2]}C) | Leitura: {nucleo_1[0]}C")

        if nucleo_2[0] >= nucleo_2[1]:
            self.logger.warning(f"[UG{self.id}] A temperatura do Núcleo Gerador 2 da UG passou do valor base! ({nucleo_2[1]}C) | Leitura: {nucleo_2[0]}C")
        if nucleo_2[0] >= 0.9*(nucleo_2[2] - nucleo_2[1]) + nucleo_2[1]:
            self.logger.critical(f"[UG{self.id}] A temperatura do Núcleo Gerador 2 da UG está muito próxima do limite! ({nucleo_2[2]}C) | Leitura: {nucleo_2[0]}C")

        if nucleo_3[0] >= nucleo_3[1]:
            self.logger.warning(f"[UG{self.id}] A temperatura do Núcleo Gerador 3 da UG passou do valor base! ({nucleo_3[1]}C) | Leitura: {nucleo_3[0]}C")
        if nucleo_3[0] >= 0.9*(nucleo_3[2] - nucleo_3[1]) + nucleo_3[1]:
            self.logger.critical(f"[UG{self.id}] A temperatura do Núcleo Gerador 3 da UG está muito próxima do limite! ({nucleo_3[2]}C) | Leitura: {nucleo_3[0]}C")

        if mancal_CR[0] >= mancal_CR[1]:
            self.logger.warning(f"[UG{self.id}] A temperatura do Mancal Casquilho Radial da UG passou do valor base! ({mancal_CR[1]}C) | Leitura: {mancal_CR[0]}C")
        if mancal_CR[0] >= 0.9*(mancal_CR[2] - mancal_CR[1]) + mancal_CR[1]:
            self.logger.critical(f"[UG{self.id}] A temperatura do Mancal Casquilho Radial da UG está muito próxima do limite! ({mancal_CR[2]}C) | Leitura: {mancal_CR[0]}C")

        if mancal_CC[0] >= mancal_CC[1]:
            self.logger.warning(f"[UG{self.id}] A temperatura do Mancal Casquilho Combinado da UG passou do valor base! ({mancal_CC[1]}C) | Leitura: {mancal_CC[0]}C")
        if mancal_CC[0] >= 0.9*(mancal_CC[2] - mancal_CC[1]) + mancal_CC[1]:
            self.logger.critical(f"[UG{self.id}] A temperatura do Mancal Casquilho Combinado da UG está muito próxima do limite! ({mancal_CC[2]}C) | Leitura: {mancal_CC[0]}C")

        if mancal_EC[0] >= mancal_EC[1]:
            self.logger.warning(f"[UG{self.id}] A temperatura do Mancal Escora Combinado da UG passou do valor base! ({mancal_EC[1]}C) | Leitura: {mancal_EC[0]}C")
        if mancal_EC[0] >= 0.9*(mancal_EC[2] - mancal_EC[1]) + mancal_EC[1]:
            self.logger.critical(f"[UG{self.id}] A temperatura do Mancal Escora Combinado da UG está muito próxima do limite! ({mancal_EC[2]}C) | Leitura: {mancal_EC[0]}C")

        if caixa_espiral[0] <= caixa_espiral[1] and caixa_espiral[0] != 0 and self.etapa_atual == UNIDADE_SINCRONIZADA:
            self.logger.warning(f"[UG{self.id}] A pressão Caixa Espiral da UG passou do valor base! ({caixa_espiral[1]:03.2f} KGf/m2) | Leitura: {caixa_espiral[0]:03.2f}")
        if caixa_espiral[0] <= caixa_espiral[2]+0.9*(caixa_espiral[1] - caixa_espiral[2]) and caixa_espiral[0] != 0 and self.etapa_atual == UNIDADE_SINCRONIZADA:
            self.logger.critical(f"[UG{self.id}] A pressão Caixa Espiral da UG está muito próxima do limite! ({caixa_espiral[2]:03.2f} KGf/m2) | Leitura: {caixa_espiral[0]:03.2f} KGf/m2")
