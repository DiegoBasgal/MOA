...
class CondicionadorBase:
    def __init__(self, leitura: LeituraBase, descr: str, gravidade: int=2, etapas: list=[], ug_id: int=None):
        self.__descr = descr
...
class CondicionadorExponencial(CondicionadorBase):
    def __init__(self, leitura: LeituraBase, descr: str, gravidade: int, valor_base: float=100, valor_limite: float=200, ordem: float = (1 / 4)):
        super().__init__(leitura, descr, gravidade)
        self.__ordem = ordem
    ...
    def valor(self) -> int | float:
        if self.leitura > self.valor_base and  self.leitura < self.valor_limite:
            ...
            return 1 if self.leitura.valor > self.valor_limite else 0

class CondicionadorExponencialReverso(CondicionadorBase):
    def __init__(self, leitura: LeituraBase, descr: str, gravidade: int, valor_base: float=16, valor_limite: float=15.5, ordem: float=2):
