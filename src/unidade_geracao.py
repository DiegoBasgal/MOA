...
class UnidadeDeGeracao:
    def __init__(self, id: "int", cfg: "dict"=None):
        ...
        self.borda_alm_diferencial: "bool" = False
    ...
    def partir(self) -> "None":
        try:
            if not se.Subestacao.status_dj_linha.valor:
                ...
                srv.Servidores.clp[f"UG{self.id}"].write_single_register(REG_UG[f"UG{self.id}"]["CMD_SINCRONISMO"][0], int(128))
    ...
    def verificar_leituras(self) -> "None":
        ...
        # WHATSAPP
        if self.l_alarme_contro_dif_grade.valor and not self.borda_alm_diferencial:
            self.borda_alm_diferencial = True
            logger.warning(f"[UG{self.id}] O Alarme de Diferencial de Grade está ativo. Favor realizar limpeza.")
        elif not self.l_alarme_contro_dif_grade.valor and self.borda_alm_diferencial:
            self.borda_alm_diferencial = False
    ...
    def carregar_leituras(self) -> "None":
        ...
        self.l_bloq_86e = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["BLOQUEIO_86E_ATUADO"], descricao=f"[UG{self.id}] Bloqueio 86E")
        self.condicionadores_essenciais.append(c.CondicionadorBase(self.l_bloq_86e, CONDIC_NORMALIZAR))

        self.l_bloq_86h = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["BLOQUEIO_86H_ATUADO"], descricao=f"[UG{self.id}] Bloqueio 86H")
        self.condicionadores_essenciais.append(c.CondicionadorBase(self.l_bloq_86h, CONDIC_NORMALIZAR))

        self.l_status_bloq_86m = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["BLOQUEIO_86M_ATUADO"], descricao=f"[UG{self.id}] Status Bloqueio 86M")
        ...
        self.condicionadores_essenciais.append(c.CondicionadorBase(self.l_botao_bloq_86eh, CONDIC_NORMALIZAR))

        self.l_rele_trip_prot_gerad = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["RELE_PROT_GERADOR_TRIP"], descricao=f"[UG{self.id}] Relé Trip Proteção Gerador")
        ...
        self.l_rt_falha_2_bloq_externo = lei.LeituraModbusBit(srv.Servidores.rv[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RV_FALHA_BLOQUEIO_EXTERNO"], descricao=f"[UG{self.id}] RT Falha Bloqueio Externo")
        ...
        self.l_vavl_bypass_discrep_senso = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["BYPASS_DISCREPANCIA_SENSORES"], descricao=f"[UG{self.id}] Válvula Bypass Discrepância Sensores")
        self.condicionadores.append(c.CondicionadorBase(self.l_vavl_bypass_discrep_senso, CONDIC_NORMALIZAR))

        self.l_val_bypass_falha_abrir = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["BYPASS_FALHA_ABRIR"], descricao=f"[UG{self.id}] Válvula Bypass Falha Abrir")
        self.condicionadores.append(c.CondicionadorBase(self.l_val_bypass_falha_abrir, CONDIC_NORMALIZAR, [UG_PARADA, UG_SINCRONIZANDO, UG_SINCRONIZADA], self))

        self.l_val_bypass_falha_fechar = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["BYPASS_FALHA_FECHAR"], descricao=f"[UG{self.id}] Válvula Bypass Falha Fechar")
        self.condicionadores.append(c.CondicionadorBase(self.l_val_bypass_falha_fechar, CONDIC_NORMALIZAR, [UG_PARADA, UG_SINCRONIZANDO, UG_SINCRONIZADA], self))

        self.l_rv_alarme_sobrefrequncia = lei.LeituraModbusBit(srv.Servidores.rv[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RV_ALARME_SOBREFREQUENCIA"], descricao=f"[UG{self.id}] RV Alarme Sobrefrequência")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_alarme_sobrefrequncia, CONDIC_NORMALIZAR))

        self.l_rv_alarme_subfrequencia = lei.LeituraModbusBit(srv.Servidores.rv[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RV_ALARME_SUBFREQUENCIA"], descricao=f"[UG{self.id}] RV Alarme Subfrequência")
        ...
        # ENTRADAS DIGITAIS
        self.l_contro_trip_dif_grade = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["CONTROLE_TRIP_DIFERENCIAL_DE_GRADE"], descricao=f"[UG{self.id}] Trip Diferencial Grade")
        self.condicionadores.append(c.CondicionadorBase(self.l_contro_trip_dif_grade, CONDIC_INDISPONIBILIZAR))

        self.l_resis_aquec_gera_falha_deslig = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["RESISTENCIA_AQUECIMENTO_GERADOR_FALHA_DESLIGAR"], descricao=f"[UG{self.id}] Resistência Aquecedor Gerador Falha Desligar")
        self.condicionadores.append(c.CondicionadorBase(self.l_resis_aquec_gera_falha_deslig, CONDIC_INDISPONIBILIZAR, [UG_SINCRONIZANDO, UG_SINCRONIZADA, UG_PARANDO], self))

        self.l_valv_borb_falha_fechar = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["BORBOLETA_FALHA_FECHAR"], descricao=f"[UG{self.id}] Válvula Borboleta Falha Fechar")
        self.condicionadores.append(c.CondicionadorBase(self.l_valv_borb_falha_fechar, CONDIC_INDISPONIBILIZAR, [UG_PARANDO, UG_PARADA], self))

        self.l_valv_borb_dicrep_senso = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["BORBOLETA_DISCREPANCIA_SENSORES"], descricao=f"[UG{self.id}] Válvula Borboleta Discrepância Sensores")
        ...
        self.l_uhrv_oleo_nv_muito_baixo = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["UHRV_OLEO_NIVEL_MUITO_BAIXO"], descricao=f"[UG{self.id}] UHRV Óleo Nível Muito Baixo")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhrv_oleo_nv_muito_baixo, CONDIC_INDISPONIBILIZAR))

        self.l_uhrv_filtro_oleo_sujo = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["UHRV_FILTRO_OLEO_SUJO"], descricao=f"[UG{self.id}] UHRV Filtro Óleo Sujo")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhrv_filtro_oleo_sujo, CONDIC_INDISPONIBILIZAR))

        self.l_uhrv_oleo_nv_muito_alto = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["UHLM_OLEO_NIVEL_MUITO_ALTO"], descricao=f"[UG{self.id}] UHRV Óleo Nível Muito Alto")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhrv_oleo_nv_muito_alto, CONDIC_INDISPONIBILIZAR))

        self.l_uhlm_oleo_nv_muito_baixo = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["UHLM_OLEO_NIVEL_MUITO_BAIXO"], descricao=f"[UG{self.id}] UHLM Óleo Nível Muito Baixo")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhlm_oleo_nv_muito_baixo, CONDIC_INDISPONIBILIZAR))

        self.l_uhlm_press_linha_lubrifi = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["UHLM_PRESSAO_LINHA_LUBRIFICACAO"], invertido=True, descricao=f"[UG{self.id}] UHLM Pressão Linha Lubrificação")
        ...
        self.l_qbag_escova_polo_pos_desgas = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["QBAG_ESCOVA_POLO_POSITIVO_DESGASTADA"], descricao=f"[UG{self.id}] QBAG Escova Polo Positivo Desgastada")
        self.condicionadores.append(c.CondicionadorBase(self.l_qbag_escova_polo_pos_desgas, CONDIC_INDISPONIBILIZAR, [UG_SINCRONIZANDO, UG_SINCRONIZADA], self))

        self.l_qbag_escova_polo_neg_desgas = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["QBAG_ESCOVA_POLO_NEGATICO_DESGASTADA"], descricao=f"[UG{self.id}] QBAG Escova Polo Negativo Desgastada")
        self.condicionadores.append(c.CondicionadorBase(self.l_qbag_escova_polo_neg_desgas, CONDIC_INDISPONIBILIZAR, [UG_SINCRONIZANDO, UG_SINCRONIZADA], self))

        self.l_rele_prot_gerad_50bf = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["RELE_PROT_GERADOR_50BF"], descricao=f"[UG{self.id}] Relé Proteção Gerador 50BF")
        self.condicionadores.append(c.CondicionadorBase(self.l_rele_prot_gerad_50bf, CONDIC_INDISPONIBILIZAR))

        self.l_disj_tps_protecao = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["DISJUNTOR_TPS_PROTECAO"], descricao=f"[UG{self.id}] Disjuntor TPS Proteção")
        self.condicionadores.append(c.CondicionadorBase(self.l_disj_tps_protecao, CONDIC_INDISPONIBILIZAR))

        self.l_uhrv_pressao_freio = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["UHRV_PRESSAO_FREIO"], descricao=f"[UG{self.id}] UHRV Pressão Freio")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhrv_pressao_freio, CONDIC_NORMALIZAR, [UG_SINCRONIZANDO, UG_SINCRONIZADA, UG_PARANDO], self))

        # UHRV
        self.l_uhrv_bomba_1_indisp = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["UHRV_BOMBA_1_INDISPONIVEL"], descricao=f"[UG{self.id}] UHRV Bomba 1 Indisponível")
        ...
        # UHLM
        self.l_uhlm_bomba_1_indisp = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["UHLM_BOMBA_1_INDISPONIVEL"], descricao=f"[UG{self.id}] UHLM Bomba 1 Indisponível")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhlm_bomba_1_indisp, CONDIC_INDISPONIBILIZAR))

        self.l_ulhm_filtro_sujo = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["UHLM_FILTRO_OLEO_SUJO"], descricao=f"[UG{self.id}] UHLM Filtro Sujo")
        self.condicionadores.append(c.CondicionadorBase(self.l_ulhm_filtro_sujo, CONDIC_INDISPONIBILIZAR))

        # RV
        self.l_rv_girando_gir_indev = lei.LeituraModbusBit(srv.Servidores.rv[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RV_FALHA_GIRANDO_SEM_REGULACAO_OU_GIRO_INDEVIDO"], descricao=f"[UG{self.id}] Falha RV Girando Sem Registro de Giro Indevido")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_girando_gir_indev, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_1_sobrefreq_inst = lei.LeituraModbusBit(srv.Servidores.rv[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RV_FALHA_SOBREFREQUENCIA_INSTANTANEA"], descricao=f"[UG{self.id}] RV Falha 1 Sobrefrequência Instantânea")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_1_sobrefreq_inst, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_1_sobrefreq_tempor = lei.LeituraModbusBit(srv.Servidores.rv[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RV_FALHA_SOBREFREQUENCIA_TEMPORIZADA"], descricao=f"[UG{self.id}] RV Falha 1 Sobrefrequência Temporizada")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_1_sobrefreq_tempor, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_1_subfreq_tempor = lei.LeituraModbusBit(srv.Servidores.rv[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RV_FALHA_SUBFREQUENCIA_TEMPORIZADA"], descricao=f"[UG{self.id}] RV Falha 1 Subfrequência Temporizada")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_1_subfreq_tempor, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_1_leit_pos_distrib = lei.LeituraModbusBit(srv.Servidores.rv[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RV_FALHA_LEITURA_POSICAO_DISTRIBUIDOR"], descricao=f"[UG{self.id}] RV Falha 1 Leitura Posição Distribuidor")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_1_leit_pos_distrib, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_1_leit_pot_ativa = lei.LeituraModbusBit(srv.Servidores.rv[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RV_FALHA_LEITURA_POTENCIA_ATIVA"], descricao=f"[UG{self.id}] RV Falha 1 Leitura Potência Ativa")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_1_leit_pot_ativa, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_1_leit_refer_pot = lei.LeituraModbusBit(srv.Servidores.rv[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RV_FALHA_LEITURA_REFERENCIA_POTENCIA"], descricao=f"[UG{self.id}] RV Falha 1 Leitura Referência Potência")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_1_leit_refer_pot, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_1_control_pos_distribu = lei.LeituraModbusBit(srv.Servidores.rv[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RV_FALHA_CONTROLE_POSICAO_DISTRIBUIDOR"], descricao=f"[UG{self.id}] RV Falha 1 Controle Posição Distribuidor")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_1_control_pos_distribu, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_1_ruido_med_veloc_princi = lei.LeituraModbusBit(srv.Servidores.rv[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RV_FALHA_RUIDO_MEDICAO_VELOCIDADE_PRINCIPAL"], descricao=f"[UG{self.id}] RV Falha 1 Ruído Medição Velocidade Principal")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_1_ruido_med_veloc_princi, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_1_ruido_med_veloc_retag = lei.LeituraModbusBit(srv.Servidores.rv[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RV_FALHA_RUIDO_MEDICAO_VELOCIDADE_RETAGUARDA"], descricao=f"[UG{self.id}] RV Falha 1 Ruído Medição Velocidade Retaguarda")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_1_ruido_med_veloc_retag, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_2_perda_med_veloc_retag = lei.LeituraModbusBit(srv.Servidores.rv[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RV_FALHA_PERDA_MEDICAO_VELOCIDADE_RETAGUARDA"], descricao=f"[UG{self.id}] RV Falha 2 Perda Medição Velocidade Retaguarda")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_2_perda_med_veloc_retag, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_2_tempo_excess_partida = lei.LeituraModbusBit(srv.Servidores.rv[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RV_FALHA_TEMPO_EXCESSIVO_PARTIDA"], descricao=f"[UG{self.id}] RV Falha 2 Tempo Excessivo Partida")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_2_tempo_excess_partida, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_2_tempo_excess_parada = lei.LeituraModbusBit(srv.Servidores.rv[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RV_FALHA_TEMPO_EXCESSIVO_PARADA"], descricao=f"[UG{self.id}] RV Falha 2 Tempo Excessivo Parada")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_2_tempo_excess_parada, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_2_dif_med_velo_princ_retag = lei.LeituraModbusBit(srv.Servidores.rv[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RV_FALHA_DIFERENCA_MEDICAO_VELOCIDADE_PRINCIPAL_E_RETAGUARDA"], descricao=f"[UG{self.id}] RV Falha 2 Diferença Medição Velocidade Principal Retaguarda")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_2_dif_med_velo_princ_retag, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_1_perda_med_velo_princ = lei.LeituraModbusBit(srv.Servidores.rv[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RV_FALHA_PERDA_MEDICAO_VELOCIDADE_PRINCIPAL"], descricao=f"[UG{self.id}] RV Falha 1 Perda Medição Velocidade Principal")
        ...
        # RT
        self.l_rt_crowbar_inativo = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_CROWBAR_INATIVO"], descricao=f"[UG{self.id}] RT Crowbar Inativo")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_crowbar_inativo, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_sobretensao = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_SOBRETENSAO"], descricao=f"[UG{self.id}] RT Alarmes 1 Sobretensão")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_sobretensao, CONDIC_INDISPONIBILIZAR))

        # self.l_rt_alar_1_subtensao = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_SUBTENSAO"], descricao=f"[UG{self.id}] RT Alarmes 1 Subtensão")
        # self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_subtensao, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_sobrefrequencia = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_SOBREFREQUENCIA"], descricao=f"[UG{self.id}] RT Alarmes 1 Sobrefrequência")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_sobrefrequencia, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_subfrequencia = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_SUBFREQUENCIA"], descricao=f"[UG{self.id}] RT Alarmes 1 Subfrequência")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_subfrequencia, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_lim_sup_pot_reativa = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_LIMITE_SUPERIOR_POTENCIA_REATIVA"], descricao=f"[UG{self.id}] RT Alarmes 1 Limite Superior Potência Reativa")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_lim_sup_pot_reativa, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_lim_inf_pot_reativa = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_LIMITE_INFERIOR_POTENCIA_REATIVA"], descricao=f"[UG{self.id}] RT Alarmes 1 Limite Inferior Potência Reativa")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_lim_inf_pot_reativa, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_lim_sup_fator_pot = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_LIMITE_SUPERIOR_FATOR_DE_POTENCIA"], descricao=f"[UG{self.id}] RT Alarmes 1 Limite Superior Fator Potência")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_lim_sup_fator_pot, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_lim_inf_fator_pot = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_LIMITE_INFERIOR_FATOR_DE_POTENCIA"], descricao=f"[UG{self.id}] RT Alarmes 1 Limite Inferior Fator Potência")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_lim_inf_fator_pot, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_variacao_tensao = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_VARIACAO_DE_TENSAO"], descricao=f"[UG{self.id}] RT Alarmes 1 Variação de Tensão")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_variacao_tensao, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_pot_ativa_reversa = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_POTENCIA_ATIVA_REVERSA"], descricao=f"[UG{self.id}] RT Alarmes 1 Potência Ativa Reversa")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_pot_ativa_reversa, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_sobrecorr_term = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_SOBRECORRENTE_TERMINAL"], descricao=f"[UG{self.id}] RT Alarmes 1 Sobrecorrente Terminal")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_sobrecorr_term, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_lim_sup_corr_excitacao = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_LIMITE_SUPERIOR_CORRENTE_EXCITACAO"], descricao=f"[UG{self.id}] RT Alarmes 1 Limite Superior Corrente Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_lim_sup_corr_excitacao, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_lim_inf_corr_exci = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_LIMITE_INFERIOR_CORRENTE_EXCITACAO"], descricao=f"[UG{self.id}] RT Alarmes 1 Limite Inferior Corrente Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_lim_inf_corr_exci, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_temp_muito_alta_rotor = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_TEMPERATURA_MUITO_ALTA_ROTOR"], descricao=f"[UG{self.id}] RT Alarmes 1 Temperatura Muito Alta Rotor")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_temp_muito_alta_rotor, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_pres_corr_exci_aus_tens_term = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_PRESENCA_DE_TENSAO_TERMINAL_COM_AUSENCIA_DE_CORRENTE_DE_EXCITACAO"], descricao=f"[UG{self.id}] RT Alarmes 1 Presença Corrente Excitação Ausente Tensão Terminal")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_pres_corr_exci_aus_tens_term, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_2_falha_contro_corr_exci = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_FALHA_CONTROLE_CORRENTE_EXCITACAO"], descricao=f"[UG{self.id}] RT Alarmes 2 Falha Controle Corrente Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_2_falha_contro_corr_exci, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_2_falha_contro_tens_term = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_FALHA_CONTROLE_TENSAO_TERMINAL"], descricao=f"[UG{self.id}] RT Alarmes 2 Falha Controle Tensão Terminal")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_2_falha_contro_tens_term, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_2_crowbar_atuado_regul_hab = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_CROWBAR_ATUADO_COM_REGULADOR_HABILITADO"], descricao=f"[UG{self.id}] RT Alarme 2 Crowbar Atuado Regulador Habilitado")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_2_crowbar_atuado_regul_hab, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_2_falha_hab_drive_excit = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_FALHA_HABILITAR_DRIVE_DE_EXCITACAO"], descricao=f"[UG{self.id}] RT Alarme 2 Falha Habilitar Drive Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_2_falha_hab_drive_excit, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_2_falha_fechar_contator_campo = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_FALHA_FECHAR_CONTATOR_DE_CAMPO"], descricao=f"[UG{self.id}] RT Alarme 2 Falha Fechar Contator Campo")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_2_falha_fechar_contator_campo, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_2_falha_corr_exci_pre_exci_ativa = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_FALHA_DE_CORRENTE_DE_EXCITACAO_COM_PRE_EXCITACAO_ATIVA"], descricao=f"[UG{self.id}] RT Alarme 2 Falha Corrente Excitação Pré Excitação Ativa")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_2_falha_corr_exci_pre_exci_ativa, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_2_perda_med_pot_reat = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_PERDA_MEDICAO_POTENCIA_REATIVA"], descricao=f"[UG{self.id}] RT Alarme 2 Perda Medição Potência Reativa")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_2_perda_med_pot_reat, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_2_perda_med_tens_term = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_PERDA_MEDICAO_TENSAO_TERMINAL"], descricao=f"[UG{self.id}] RT Alarme 2 Perda Medição Tensão Terminal")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_2_perda_med_tens_term, CONDIC_NORMALIZAR))

        self.l_rt_alar_2_perda_med_corr_exci = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_PERDA_MEDICAO_CORRENTE_EXCITACAO"], descricao=f"[UG{self.id}] RT Alarme 2 Perda Medição Corrente Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_2_perda_med_corr_exci, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_ruido_intrumen_reat = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_RUIDO_INSTRUMENTACAO_DE_REATIVO"], descricao=f"[UG{self.id}] RT Alarme 2 Ruído Leitura Intrumentador Reativo")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_ruido_intrumen_reat, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_ruido_intrumen_tensao = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_RUIDO_INSTRUMENTACAO_DE_TENSAO"], descricao=f"[UG{self.id}] RT Alarme 2 Ruído Leitura Intrumentador Tensão")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_ruido_intrumen_tensao, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_ruido_intrumen_exci_princ = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_RUIDO_INSTRUMENTACAO_DE_EXCITACAO_PRINCIPAL"], descricao=f"[UG{self.id}] RT Alarme 2 Ruído Leitura Intrumentador Excitação Principal")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_ruido_intrumen_exci_princ, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_ruido_intrumen_exci_retag = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_RUIDO_INSTRUMENTACAO_DE_EXCITACAO_RETAGUARDA"], descricao=f"[UG{self.id}] RT Alarme 2 Ruído Leitura Intrumentador Excitação Retaguarda")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_ruido_intrumen_exci_retag, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_sobretensao = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_SOBRETENSAO"], descricao=f"[UG{self.id}] RT Falha 1 Sobretensão")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_sobretensao, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_subtensao = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_SUBTENSAO"], descricao=f"[UG{self.id}] RT Falha 1 Subtensão")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_subtensao, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_sobrefreq = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_SOBREFREQUENCIA"], descricao=f"[UG{self.id}] RT Falha 1 Sobrefrequência")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_sobrefreq, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_subfreq = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_SUBFREQUENCIA"], descricao=f"[UG{self.id}] RT Falha 1 Subfrequência")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_subfreq, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_lim_sup_pot_reat = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_LIMITE_SUPERIOR_POTENCIA_REATIVA"], descricao=f"[UG{self.id}] RT Falha 1 Limite Superior Potência Reativa")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_lim_sup_pot_reat, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_lim_inf_pot_reat = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_LIMITE_INFERIOR_POTENCIA_REATIVA"], descricao=f"[UG{self.id}] RT Falha 1 Limite Inferior Potência Reativa")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_lim_inf_pot_reat, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_lim_sup_fator_pot = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_LIMITE_SUPERIOR_FATOR_DE_POTENCIA"], descricao=f"[UG{self.id}] RT Falha 1 Limite Superior Fator Potência")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_lim_sup_fator_pot, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_lim_inf_fator_pot = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_LIMITE_INFERIOR_FATOR_DE_POTENCIA"], descricao=f"[UG{self.id}] RT Falha 1 Limite Inferior Fator Potência")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_lim_inf_fator_pot, CONDIC_NORMALIZAR))

        self.l_rt_falha_1_sobretensao_inst = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_SOBRETENSAO_INSTANTANEA"], descricao=f"[UG{self.id}] RT Falha 1 Sobretensão Instantânea")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_sobretensao_inst, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_variacao_tensao = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_VARIACAO_DE_TENSAO"], descricao=f"[UG{self.id}] RT Falha 1 Variação Tensão")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_variacao_tensao, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_pot_ativ_reversa = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_POTENCIA_ATIVA_REVERSA"], descricao=f"[UG{self.id}] RT Falha 1 Potência Ativa Reversa")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_pot_ativ_reversa, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_sobrecorr_term = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_SOBRECORRENTE_TERMINAL"], descricao=f"[UG{self.id}] RT Falha 1 Sobrecorrente Terminal")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_sobrecorr_term, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_lim_sup_corr_exci = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_LIMITE_SUPERIOR_CORRENTE_EXCITACAO"], descricao=f"[UG{self.id}] RT Falha 1 Limite Superior Corrente Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_lim_sup_corr_exci, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_lim_inf_corr_exci = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_LIMITE_INFERIOR_CORRENTE_EXCITACAO"], descricao=f"[UG{self.id}] RT Falha 1 Limite Inferior Corrente Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_lim_inf_corr_exci, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_lim_sup_tensao_exci = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_LIMITE_SUPERIOR_TENSAO_EXCITACAO"], descricao=f"[UG{self.id}] RT Falha 1 Limite Superior Tensão Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_lim_sup_tensao_exci, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_lim_inf_tensao_exci = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_LIMITE_INFERIOR_TENSAO_EXCITACAO"], descricao=f"[UG{self.id}] RT Falha 1 Limite Inferior Tensão Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_lim_inf_tensao_exci, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_2_temp_muito_alta_rotor = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_TEMPERATURA_MUITO_ALTA_ROTOR"], descricao=f"[UG{self.id}] RT Falha 2 Temperatura Muito Alta Rotor")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_2_temp_muito_alta_rotor, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_2_pres_tens_term_aus_corr_exci = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_PRESENCA_DE_TENSAO_TERMINAL_COM_AUSENCIA_DE_CORRENTE_DE_EXCITACAO"], descricao=f"[UG{self.id}] RT Falha 2 Presença Tensão Terminal Ausente Corrente Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_2_pres_tens_term_aus_corr_exci, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_2_pres_corr_exci_aus_tens_term = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_PRESENCA_DE_CORRENTE_DE_EXCITACAO_COM_AUXENCIA_DE_TENSAO_TERMINAL"], descricao=f"[UG{self.id}] RT Falha 2 Presença Corrente Excitação Ausente Tensão Terminal")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_2_pres_corr_exci_aus_tens_term, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_2_control_corr_exci = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_FALHA_CONTROLE_CORRENTE_EXCITACAO"], descricao=f"[UG{self.id}] RT Falha 2 Controle Corrente Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_2_control_corr_exci, CONDIC_INDISPONIBILIZAR, [UG_SINCRONIZANDO, UG_SINCRONIZADA], self))

        self.l_rt_falha_2_tensao_term = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_FALHA_CONTROLE_TENSAO_TERMINAL"], descricao=f"[UG{self.id}] RT Falha 2 Tensão Terminal")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_2_tensao_term, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_2_crowbar_atuado_regu_hab = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_CROWBAR_ATUADO_COM_REGULADOR_HABILITADO"], descricao=f"[UG{self.id}] RT Falha 2 Crowbar Atuado Regulador Habilitado")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_2_crowbar_atuado_regu_hab, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_2_hab_drive_exci = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_FALHA_HABILITAR_DRIVE_DE_EXCITACAO_LOGICA_DE_DISPARO"], descricao=f"[UG{self.id}] RT Falha 2 Habilitar Drive Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_2_hab_drive_exci, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_2_fechar_contator_campo = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALAME_FALHA_FECHAR_CONTATOR_DE_CAMPO"], descricao=f"[UG{self.id}] RT Falha 2 Fechar Contator Campo")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_2_fechar_contator_campo, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_2_corr_exci_pre_exci_ativa = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_FALHA_DE_CORRENTE_DE_EXCITACAO_COM_PRE_EXCITACAO_ATIVA"], descricao=f"[UG{self.id}] RT Falha 2 Corrente Excitação Pré Excitada Ativa")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_2_corr_exci_pre_exci_ativa, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_2_excess_pre_exci = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_TEMPO_EXCESSIVO_DE_PRE_EXCITACAO"], descricao=f"[UG{self.id}] RT Falha 2 Excessivo Pré Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_2_excess_pre_exci, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_2_excess_parada = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_TEMPO_EXCESSIVO_DE_PARADA"], descricao=f"[UG{self.id}] RT Falha 2 Excessivo Parada")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_2_excess_parada, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_3_perda_med_pot_reativa = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_TEMPO_EXCESSIVO_DE_PARTIDA"], descricao=f"[UG{self.id}] RT Falha 3 Perda Medição Potência Reativa")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_3_perda_med_pot_reativa, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_3_perda_med_tensao_term = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_PERDA_MEDICAO_TENSAO_TERMINAL"], descricao=f"[UG{self.id}] RT Falha 3 Perda Medição Tensão Terminal")
        ...
        self.l_rt_falha_3_ruido_leit_instrum_reativo = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_RUIDO_INSTRUMENTACAO_DE_REATIVO"], descricao=f"[UG{self.id}] RT Falha 3 Ruído Leitura Instrumentador Reativo")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_3_ruido_leit_instrum_reativo, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_3_ruido_leit_instrum_tensao = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_RUIDO_INSTRUMENTACAO_DE_TENSAO"], descricao=f"[UG{self.id}] RT Falha 3 Ruído Leitura Instrumentador Tensão")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_3_ruido_leit_instrum_tensao, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_3_ruido_leit_instrum_principal = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_RUIDO_INSTRUMENTACAO_DE_EXCITACAO_PRINCIPAL"], descricao=f"[UG{self.id}] RT Falha 3 Ruído Leitura Instrumentador Principal")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_3_ruido_leit_instrum_principal, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_3_ruido_leit_instrum_retag = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_FALHA_RUIDO_INSTRUMENTACAO_DE_EXCITACAO_RETAGUARDA"], descricao=f"[UG{self.id}] RT Falha 3 Ruído Leitura Instrumentador Retaguarda")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_3_ruido_leit_instrum_retag, CONDIC_INDISPONIBILIZAR))

        # ENTRADAS ANALÓGICAS
        self.l_tristor_temp_muito_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["TIRISTORES_TEMPERATURA_MUITO_ALTA"], descricao=f"[UG{self.id}] Tristores Temperatura Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_tristor_temp_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_crowbar_temp_muito_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["CROWBAR_TEMPERATURA_MUITO_ALTA"], descricao=f"[UG{self.id}] Crowbar Temperatura Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_crowbar_temp_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_uhrv_temp_oleo_muito_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["UHRV_TEMPERATURA_OLEO_MUITO_ALTA"], descricao=f"[UG{self.id}] UHRV Óleo Temperatura Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhrv_temp_oleo_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_gera_fase_a_temp_muito_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["GERADOR_FASE_A_TEMPERATURA_MUITO_ALTA"], descricao=f"[UG{self.id}] Gerador Fase A Temperatura Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_gera_fase_a_temp_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_gera_fase_b_temp_muito_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["GERADOR_FASE_B_TEMPERATURA_MUITO_ALTA"], descricao=f"[UG{self.id}] Gerador Fase B Temperatura Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_gera_fase_b_temp_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_gera_fase_c_temp_muito_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["GERADOR_FASE_C_TEMPERATURA_MUITO_ALTA"], descricao=f"[UG{self.id}] Gerador Fase C Temperatura Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_gera_fase_c_temp_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_gera_nucleo_1_temp_muito_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["GERADOR_NUCLEO_1_TEMPERATURA_MUITO_ALTA"], descricao=f"[UG{self.id}] Gerador Núcleo 1 Temperatura Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_gera_nucleo_1_temp_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_gera_nucleo_2_temp_muito_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["GERADOR_NUCLEO_2_TEMPERATURA_MUITO_ALTA"], descricao=f"[UG{self.id}] Gerador Núcleo 2 Temperatura Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_gera_nucleo_2_temp_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_gera_nucleo_3_temp_muito_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["GERADOR_NUCLEO_3_TEMPERATURA_MUITO_ALTA"], descricao=f"[UG{self.id}] Gerador Núcleo 3 Temperatura Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_gera_nucleo_3_temp_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_mancal_guia_casq_temp_muito_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["MANCAL_GUIA_CASQUILHO_TEMPERATURA_MUITO_ALTA"], descricao=f"[UG{self.id}] Mancal Guia Casquilho Temperatura Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_mancal_guia_casq_temp_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_mancal_comb_casq_temp_muito_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["MANCAL_COMBINADO_CASQUILHO_1_TEMPERATURA_MUITO_ALTA"], descricao=f"[UG{self.id}] Mancal Combinado Casquilho Temperatura Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_mancal_comb_casq_temp_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_mancal_comb_esc_temp_muito_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["MANCAL_COMBINADO_ESCORA_TEMPERATURA_MUITO_ALTA"], descricao=f"[UG{self.id}] Mancal Combinado Escora Temperatura Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_mancal_comb_esc_temp_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_uhrv_press_oleo_falha_leitura = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["UHRV_PRESSAO_OLEO_FALHA_LEITURA"], descricao=f"[UG{self.id}] UHRV Pressão Óleo Falha Leitura")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhrv_press_oleo_falha_leitura, CONDIC_INDISPONIBILIZAR))

        self.l_uhrv_press_oleo_muito_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["UHRV_PRESSAO_OLEO_MUITO_ALTA"], descricao=f"[UG{self.id}] UHRV Pressão Óleo Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhrv_press_oleo_muito_alta, CONDIC_INDISPONIBILIZAR, [UG_SINCRONIZANDO, UG_SINCRONIZADA], self))

        self.l_uhrv_press_oleo_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["UHRV_PRESSAO_OLEO_ALTA"], descricao=f"[UG{self.id}] UHRV Pressão Óleo Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhrv_press_oleo_alta, CONDIC_INDISPONIBILIZAR, [UG_SINCRONIZANDO, UG_SINCRONIZADA], self))

        self.l_falha_fechar_distrib = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["RV_FALHA_AO_FECHAR_DISTRIBUIDOR"], descricao=f"[UG{self.id}] Falha Fechamento Distribuidor")
        self.condicionadores.append(c.CondicionadorBase(self.l_falha_fechar_distrib, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_partir = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["RV_FALHA_AO_PARTIR"], descricao=f"[UG{self.id}] RV Falha Partir")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_partir, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_desab = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["RV_FALHA_AO_DESABILITAR"], descricao=f"[UG{self.id}] RV Falha Desabilitar")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_desab, CONDIC_INDISPONIBILIZAR, [UG_PARANDO, UG_PARADA], self))

        self.l_rv_falha_parar_maqu = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["RV_FALHA_AO_PARAR_MAQUINA"], descricao=f"[UG{self.id}] RV Falha Parar Máquina")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_parar_maqu, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_partir = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["RT_FALHA_AO_PARTIR"], descricao=f"[UG{self.id}] RT Falha Partir")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_partir, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_desab = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["RT_FALHA_AO_DESABILITAR"], descricao=f"[UG{self.id}] RT Falha Desabilitar")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_desab, CONDIC_INDISPONIBILIZAR, [UG_PARANDO, UG_PARADA], self))

        self.l_rv_falha_hab = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["RT_FALHA_AO_HABILITAR"], descricao=f"[UG{self.id}] RV Falha Habilitar")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_hab, CONDIC_INDISPONIBILIZAR))

        self.l_uhrv_bomba_1_falha_ligar = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["UHRV_BOMBA_1_FALHA_AO_LIGAR"], descricao=f"[UG{self.id}] UHRV Falha Ligar")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhrv_bomba_1_falha_ligar, CONDIC_INDISPONIBILIZAR, [UG_SINCRONIZANDO, UG_SINCRONIZADA], self))

        self.l_uhlm_bomba_1_falha_ligar = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["UHLM_BOMBA_1_FALHA_AO_LIGAR"], descricao=f"[UG{self.id}] UHLM Bomba 1 Falha Ligar")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhlm_bomba_1_falha_ligar, CONDIC_INDISPONIBILIZAR, [UG_SINCRONIZANDO, UG_SINCRONIZADA], self))

        self.l_uhlm_bomba_1_falha_deslig = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["UHLM_BOMBA_1_FALHA_AO_DESLIGAR"], descricao=f"[UG{self.id}] UHLM Bomba 1 Falha Desligar")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhlm_bomba_1_falha_deslig, CONDIC_INDISPONIBILIZAR, [UG_SINCRONIZANDO, UG_SINCRONIZADA], self))
        
        self.l_uhlm_bomba_1_falha_pressurizar = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["UHLM_BOMBA_1_FALHA_AO_PRESSURIZAR"], descricao=f"[UG{self.id}] UHLM Bomba 1 Falha Pressurizar")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhlm_bomba_1_falha_pressurizar, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_pres_tens_term_aus_corr_exci = lei.LeituraModbusBit(srv.Servidores.rt[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RT_ALARME_PRESENCA_DE_TENSAO_TERMINAL_COM_AUSENCIA_DE_CORRENTE_DE_EXCITACAO"], descricao=f"[UG{self.id}] RT Alarmes 1 Presença Tensão Terminal Ausente Corrente Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_pres_tens_term_aus_corr_exci, CONDIC_INDISPONIBILIZAR))

        ## WHATSAPP + VOIP
        self.l_cmd_uhrv_modo_manuten = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["CMD_UHRV_MODO_MANUTENCAO"], descricao=f"[UG{self.id}] UHRV Comando Modo Manutenção")
        self.l_cmd_uhlm_modo_manuten = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["CMD_UHLM_MODO_MANUTENCAO"], descricao=f"[UG{self.id}] UHLM Comando Modo Manutenção")
        self.l_resis_quec_gerador_falha_ligar = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["RESISTENCIA_AQUECIMENTO_GERADOR_FALHA_LIGAR"], descricao=f"[UG{self.id}] ")

        self.l_falha_leit_temp_tristores = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["TIRISTORES_TEMPERATURA_FALHA_LEITURA"], descricao=f"[UG{self.id}] Tristores Temperatura Falha Leitura")
        self.l_falha_leit_temp_crowbar = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["CROWBAR_TEMPERATURA_FALHA_LEITURA"], descricao=f"[UG{self.id}] Crowbar Temperatura Falha Leitura")
        self.l_falha_leit_temp_uhrv_temp_oleo = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["UHRV_TEMPERATURA_OLEO_FALHA_LEITURA"], descricao=f"[UG{self.id}] UHRV Temperatura Óleo Falha Leitura")
        self.l_falha_leit_temp_gerad_fase_a = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["GERADOR_FASE_A_TEMPERATURA_FALHA_LEITURA"], descricao=f"[UG{self.id}] Gerador Fase A Temperatura Falha Leitura")
        self.l_falha_leit_temp_gerad_fase_b = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["GERADOR_FASE_B_TEMPERATURA_FALHA_LEITURA"], descricao=f"[UG{self.id}] Gerador Fase B Temperatura Falha Leitura")
        self.l_falha_leit_temp_gerad_fase_c = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["GERADOR_FASE_C_TEMPERATURA_FALHA_LEITURA"], descricao=f"[UG{self.id}] Gerador Fase C Temperatura Falha Leitura")
        self.l_falha_leit_temp_gerad_nucleo_1 = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["GERADOR_NUCLEO_1_TEMPERATURA_FALHA_LEITURA"], descricao=f"[UG{self.id}] Gerador Núcleo 1 Temperatura Falha Leitura")
        self.l_falha_leit_temp_gerad_nucleo_2 = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["GERADOR_NUCLEO_2_TEMPERATURA_FALHA_LEITURA"], descricao=f"[UG{self.id}] Gerador Núcleo 2 Temperatura Falha Leitura")
        self.l_falha_leit_temp_gerad_nucleo_3 = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["GERADOR_NUCLEO_3_TEMPERATURA_FALHA_LEITURA"], descricao=f"[UG{self.id}] Gerador Núcleo 3 Temperatura Falha Leitura")
        self.l_falha_leit_temp_mancal_guia_casq = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["MANCAL_GUIA_CASQUILHO_TEMPERATURA_FALHA_LEITURA"], descricao=f"[UG{self.id}] Mancal Guia Casquilho temperatura Falha Leitura")
        self.l_falha_leit_temp_mancal_comb_esc = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["MANCAL_COMBINADO_CASQUILHO_1_TEMPERATURA_FALHA_LEITURA"], descricao=f"[UG{self.id}] Mancal Combinado Escora Temperatura Falha Leitura")

        ## WHATSAPP
        self.l_rv_pot_nula = lei.LeituraModbusBit(srv.Servidores.rv[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RV_RELE_POTENCIA_NULA"], descricao=f"[UG{self.id}] RV Potência Nula")
        self.l_uhrv_bomba_defeito = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["PRTVA_UHRV_BOMBA_DEFEITO"], descricao=f"[UG{self.id}] UHRV Bomba Defeito")
        self.l_uhlm_bomba_defeito = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["PRTVA_UHLM_BOMBA_DEFEITO"], descricao=f"[UG{self.id}] UHLM Bomba Defeito")
        self.l_dispo_prot_surto = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["PRTVA_DISPOSITIVO_PROTECAO_DE_SURTO"], descricao=f"[UG{self.id}] Dispositivo Proteção Surto")
        self.l_rt_selec_modo_controle_isol = lei.LeituraModbusBit(srv.Servidores.rv[f"UG{self.id}"], REG_RTV[f"UG{self.id}"]["RV_SELECIONA_MODO_CONTROLE_ISOLADO"], descricao=f"[UG{self.id}] RT Selecionado Modo Controle Isolado")
        self.l_resis_aquec_gerad_defeito = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["UG_RESISTENCIA_AQUEC_GERADOR_DEFEITO"], descricao=f"[UG{self.id}] Resistência Aquecimento Gerador Defeito")

        self.l_crowbar_temp_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["CROWBAR_TEMPERATURA_ALTA"], descricao=f"[UG{self.id}] Temperatura Alta")
        self.l_tristores_temp_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["TIRISTORES_TEMPERATURA_ALTA"], descricao=f"[UG{self.id}] Temperatura Alta")
        self.l_uhrv_oleo_temp_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["UHRV_TEMPERATURA_OLEO_ALTA"], descricao=f"[UG{self.id}] Temperatura Alta")
        self.l_gerad_fase_a_temp_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["GERADOR_FASE_A_TEMPERATURA_ALTA"], descricao=f"[UG{self.id}] Gerador Fase A Temperatura Alta")
        self.l_gerad_fase_b_temp_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["GERADOR_FASE_B_TEMPERATURA_ALTA"], descricao=f"[UG{self.id}] Gerador Fase B Temperatura Alta")
        self.l_gerad_fase_c_temp_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["GERADOR_FASE_C_TEMPERATURA_ALTA"], descricao=f"[UG{self.id}] Gerador Fase C Temperatura Alta")
        self.l_gerad_nucleo_1_temp_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["GERADOR_NUCLEO_1_TEMPERATURA_ALTA"], descricao=f"[UG{self.id}] Gerador Núcleo 1 Temperatura Alta")
        self.l_gerad_nucleo_2_temp_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["GERADOR_NUCLEO_2_TEMPERATURA_ALTA"], descricao=f"[UG{self.id}] Gerador Núcleo 2 Temperatura Alta")
        self.l_gerad_nucleo_3_temp_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["GERADOR_NUCLEO_3_TEMPERATURA_ALTA"], descricao=f"[UG{self.id}] Gerador Núcleo 3 Temperatura Alta")
        self.l_mancal_guia_casq_temp_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["MANCAL_GUIA_CASQUILHO_TEMPERATURA_ALTA"], descricao=f"[UG{self.id}] Mancal Guia Casquilho Temperatura Alta")
        self.l_mancal_comb_esc_temp_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["MANCAL_COMBINADO_CASQUILHO_1_TEMPERATURA_ALTA"], descricao=f"[UG{self.id}] Mancal Combinado Escora Temperatura Alta")
        self.l_mancal_comb_casq_temp_alta = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["MANCAL_COMBINADO_ESCORA_TEMPERATURA_ALTA"], descricao=f"[UG{self.id}] Mancal Combinado Casquilho Temperatura Alta")

        self.l_rv_modo_manut = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["RV_MODO_MANUTENCAO"], descricao=f"[UG{self.id}] RV Modo Manutenção")
        self.l_uhlm_unidade_manut = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["UHLM_UNIDADE_MANUTENCAO"], descricao=f"[UG{self.id}] UHLM Unidade Manutenção")
        self.l_sinal_nv_jusante_alto = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["SINAL_NIVEL_JUSANTE_MUITO_ALTA"], descricao=f"[UG{self.id}] Nível Jusante Sinal Alto")
        self.l_uhrv_pressao_oleo_baixa = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["UHRV_PRESSAO_OLEO_BAIXA"], descricao=f"[UG{self.id}] UHRV Pressão Óleo Baixa")
        self.l_falha_leit_nv_jusante = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["SINAL_NIVEL_JUSANTE_FALHA_LEITURA"], descricao=f"[UG{self.id}] Nível Jusante Falha Leitura")
        self.l_sinal_nv_jusante_muito_alto = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["SINAL_NIVEL_JUSANTE_MUITO_ALTA"], descricao=f"[UG{self.id}] Nível Jusante Sinal Muito Alto")
        self.l_uhrv_pressao_oleo_muito_baixa = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["UHRV_PRESSAO_OLEO_MUITO_BAIXA"], descricao=f"[UG{self.id}] UHRV Pressão Óleo Muito Baixa")
        self.l_sinal_nv_jusante_muito_baixo = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["SINAL_NIVEL_JUSANTE_MUITO_BAIXA"], descricao=f"[UG{self.id}] Nível Jusante Sinal Muito Baixo")

        self.l_sinal_nv_jusante_baixo = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["SINAL_NIVEL_JUSANTE_BAIXA"], descricao=f"[UG{self.id}] Nível Jusante Sinal Baixo")
        self.l_alarme_contro_dif_grade = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["CONTROLE_ALARME_DIFERENCIAL_DE_GRADE"], descricao=f"[UG{self.id}] Alarme Controle Diferencial Grade")

        self.l_urhv_press_crit = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["UHRV_PRESSAO_CRITICA"], invertido=True, descricao=f"[UG{self.id}] UHRV Pressão Crítica")
        ...
        self.l_uhlm_fluxo_troc_calor = lei.LeituraModbusBit(srv.Servidores.clp[f"UG{self.id}"], REG_UG[f"UG{self.id}"]["UHLM_FLUXO_TROCADOR_DE_CALOR"], invertido=True, descricao=f"[UG{self.id}] UHLM Fluxo Trocador Calor")
