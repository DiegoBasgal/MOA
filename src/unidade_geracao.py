...
class UnidadeDeGeracao:
    ...
    def desabilitar_manutencao(self) -> "bool":
        try:
            res = esc.EscritaModBusBit.escrever_bit(self.clp[f"UG{self.id}"], REG["UG"][f"UG{self.id}_CD_CMD_CONTROLE_POTENCIA_MANUAL"], valor=1, descricao=f"UG{self.id}_CD_CMD_CONTROLE_POTENCIA_MANUAL")

            res = esc.EscritaModBusBit.escrever_bit(self.clp[f"UG{self.id}"], REG["UG"][f"UG{self.id}_CD_CMD_RV_MANUTENCAO"], valor=0, descricao=f"UG{self.id}_CD_CMD_RV_MANUTENCAO")
            res = esc.EscritaModBusBit.escrever_bit(self.clp[f"UG{self.id}"], REG["UG"][f"UG{self.id}_CD_CMD_RV_AUTOMATICO"], valor=1, descricao=f"UG{self.id}_CD_CMD_RV_AUTOMATICO")
            res = esc.EscritaModBusBit.escrever_bit(self.clp[f"UG{self.id}"], REG["UG"][f"UG{self.id}_CD_CMD_UHLM_MODO_AUTOMATICO"], valor=1, descricao=f"UG{self.id}_CD_CMD_UHLM_MODO_AUTOMATICO")
            # res = esc.EscritaModBusBit.escrever_bit(self.clp[f"UG{self.id}"], REG["UG"][f"UG{self.id}_CD_CMD_UHRV_MODO_MANUTENCAO"], 0, descricao=f"UG{self.id}_CD_CMD_UHRV_MODO_MANUTENCAO") -> Está mandando comando de parada para a máquina | TODO Verificar com a Automatic
            # res = esc.EscritaModBusBit.escrever_bit(self.clp[f"UG{self.id}"], REG["UG"][f"UG{self.id}_CD_CMD_UHRV_MODO_AUTOMATICO"], 1, descricao=f"UG{self.id}_CD_CMD_UHRV_MODO_AUTOMATICO") -> Está mandando comando de parada para a máquina | TODO Verificar com a Automatic
            return res
    ...
    def partir(self) -> "None":
        try:
            if not se.Subestacao.status_dj_linha.valor:
                ...
                esc.EscritaModBusBit.escrever_bit(self.clp[f"UG{self.id}"], REG["UG"][f"UG{self.id}_CD_CMD_REARME_FALHAS"], valor=1)
                # esc.EscritaModBusBit.escrever_bit(self.clp[f"UG{self.id}"], REG["UG"][f"UG{self.id}_CD_CMD_SINCRONISMO"], valor=1)
    ...
    def parar(self) -> "bool":
        try:
            if not self.etapa == UG_PARADA:
                ...
                res = esc.EscritaModBusBit.escrever_bit(self.clp[f"UG{self.id}"], REG["UG"][f"UG{self.id}_CD_CMD_PARADA_TOTAL"], 1, descricao=f"UG{self.id}_CD_CMD_PARADA_TOTAL")
                return res
    ...
    def acionar_trip_logico(self) -> "bool":
        try:
            ...
            res = esc.EscritaModBusBit.escrever_bit(self.clp[f"UG{self.id}"], REG["UG"][f"UG{self.id}_CD_CMD_PARADA_EMERGENCIA"], 1, descricao=f"UG{self.id}_CD_CMD_PARADA_EMERGENCIA")
            return res
    ...
    def remover_trip_logico(self) -> "bool":
        try:
            ...
            res = esc.EscritaModBusBit.escrever_bit(self.clp[f"UG{self.id}"], REG["UG"][f"UG{self.id}_CD_CMD_REARME_FALHAS"], 1, descricao=f"UG{self.id}_CD_CMD_REARME_FALHAS")
            return res
    ...
    def remover_trip_eletrico(self) -> "bool":
        try:
            if not self.__leitura_dj_linha.valor:
                ...
                esc.EscritaModBusBit.escrever_bit(self.clp["SA"], REG["SA"]["SA_CD_DISJ_LINHA_FECHA"], 1, descricao="SA_CD_DISJ_LINHA_FECHA")
    ...
    def carregar_leituras(self) -> "None":
        ...
        self.l_tmp_fase_r = lei.LeituraModbus(
            self.clp[f"UG{self.id}"],
            REG[f"UG{self.id}"]["EA_TEMPERATURA_GERADOR_FASE_A"],
            escala=0.001,
            op=4
        )
        ...
        self.l_tmp_fase_s = lei.LeituraModbus(
            self.clp[f"UG{self.id}"],
            REG[f"UG{self.id}"]["EA_TEMPERATURA_GERADOR_FASE_B"],
            escala=0.001,
            op=4
        )
        ...
        self.l_tmp_fase_t = lei.LeituraModbus(
            self.clp[f"UG{self.id}"],
            REG[f"UG{self.id}"]["EA_TEMPERATURA_GERADOR_FASE_C"],
            escala=0.001,
            op=4
        )
        ...
        self.l_tmp_nucleo_gerador_1 = lei.LeituraModbus(
            self.clp[f"UG{self.id}"],
            REG[f"UG{self.id}"]["EA_TEMPERATURA_GERADOR_NUCLEO_1"],
            escala=0.001,
            op=4
        )
        ...
        self.l_tmp_nucleo_gerador_2 = lei.LeituraModbus(
            self.clp[f"UG{self.id}"],
            REG[f"UG{self.id}"]["EA_TEMPERATURA_GERADOR_NUCLEO_2"],
            escala=0.001,
            op=4
        )
        ...
        self.l_tmp_nucleo_gerador_3 = lei.LeituraModbus(
            self.clp[f"UG{self.id}"],
            REG[f"UG{self.id}"]["EA_TEMPERATURA_GERADOR_NUCLEO_3"],
            escala=0.001,
            op=4
        )
        ...
        self.l_tmp_mancal_casq_rad = lei.LeituraModbus(
            self.clp[f"UG{self.id}"],
            REG[f"UG{self.id}"]["EA_TEMPERATURA_MANCAL_GUIA_CASQUILHO"],
            escala=0.001,
            op=4
        )
        ...
        self.l_tmp_mancal_casq_comb = lei.LeituraModbus(
            self.clp[f"UG{self.id}"],
            REG[f"UG{self.id}"]["EA_TEMPERATURA_MANCAL_COMBINADO_CASQUILHO"],
            escala=0.001,
            op=4
        )
        ...
        self.l_tmp_mancal_escora_comb = lei.LeituraModbus(
            self.clp[f"UG{self.id}"],
            REG[f"UG{self.id}"]["EA_TEMPERATURA_MANCAL_COMBINADO_ESCORA"],
            escala=0.001,
            op=4
        )
        ...
        self.l_rele_bloq_86eh = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_PRTVA_RELE_BLOQUEIO_86EH"], descricao=f"[UG{self.id}] Relé Bloqueio 86EH")
        self.condicionadores_essenciais.append(c.CondicionadorBase(self.l_rele_bloq_86eh, CONDIC_NORMALIZAR))

        self.l_bloq_86e = lei.LeituraModbusCoil(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_STT_BLOQUEIO_86E"], descricao=f"[UG{self.id}] Bloqueio 86E")
        self.condicionadores_essenciais.append(c.CondicionadorBase(self.l_bloq_86e, CONDIC_NORMALIZAR))

        self.l_bloq_86h = lei.LeituraModbusCoil(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_STT_BLOQUEIO_86H"], descricao=f"[UG{self.id}] Bloqueio 86H")
        self.condicionadores_essenciais.append(c.CondicionadorBase(self.l_bloq_86h, CONDIC_NORMALIZAR))

        self.l_status_bloq_86m = lei.LeituraModbusCoil(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_STT_BLOQUEIO_86M"], descricao=f"[UG{self.id}] Status Bloqueio 86M")
        self.condicionadores_essenciais.append(c.CondicionadorBase(self.l_status_bloq_86m, CONDIC_NORMALIZAR))

        self.l_rt_falha_2_bloq_externo = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_2_BLOQ_EXTERNO"], descricao=f"[UG{self.id}] RT Falha 2 Bloqueio Externo")
        self.condicionadores_essenciais.append(c.CondicionadorBase(self.l_rt_falha_2_bloq_externo, CONDIC_NORMALIZAR))

        self.l_rele_trip_prot_gerad = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_PRTVA_RELE_PROT_GERADOR_TRIP"], descricao=f"[UG{self.id}] Relé Trip Proteção Gerador")
        self.condicionadores_essenciais.append(c.CondicionadorBase(self.l_rele_trip_prot_gerad, CONDIC_NORMALIZAR))

        self.l_rv_trip = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_PRTVA_RV_TRIP"], descricao=f"[UG{self.id}] RV Trip")
        self.condicionadores_essenciais.append(c.CondicionadorBase(self.l_rv_trip, CONDIC_NORMALIZAR))

        self.l_rt_trip = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_PRTVA_RT_TRIP"], descricao=f"[UG{self.id}] RT Trip")
        self.condicionadores_essenciais.append(c.CondicionadorBase(self.l_rt_trip, CONDIC_NORMALIZAR))


        ## CONDICIONADORES NORMALIZAR
        self.l_val_bypass_falha_abrir = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_BYPASS_FALHA_ABRIR"], descricao=f"[UG{self.id}] Válvula Bypass Falha Abrir")
        self.condicionadores.append(c.CondicionadorBase(self.l_val_bypass_falha_abrir, CONDIC_NORMALIZAR))

        self.l_val_bypass_falha_fechar = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_BYPASS_FALHA_FECHAR"], descricao=f"[UG{self.id}] Válvula Bypass Falha Fechar")
        self.condicionadores.append(c.CondicionadorBase(self.l_val_bypass_falha_fechar, CONDIC_NORMALIZAR))

        self.l_rv_alarme_sobrefrequncia = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RV_ALARME_SOBREFREQUENCIA"], descricao=f"[UG{self.id}] RV Alarme Sobrefrequência")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_alarme_sobrefrequncia, CONDIC_NORMALIZAR))

        self.l_rv_alarme_subfrequencia = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RV_ALARME_SUBFREQUENCIA"], descricao=f"[UG{self.id}] RV Alarme Subfrequência")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_alarme_subfrequencia, CONDIC_NORMALIZAR))

        self.l_rv_falha_1 = lei.LeituraModbusCoil(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RV_FALHA_1"], descricao=f"[UG{self.id}] RV Falha 1")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_1, CONDIC_NORMALIZAR))

        # TODO -> Verificar
        # self.l_botao_bloq_86eh = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_PRTVA_BOTAO_BLOQUEIO_86EH"], descricao=f"[UG{self.id}] Botão Bloqueio 86EH")
        # self.condicionadores.append(c.CondicionadorBase(self.l_botao_bloq_86eh, CONDIC_NORMALIZAR))

        self.l_valv_borb_falha_abertura = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_BROBOLETA_FALHA_ABRIR"], descricao=f"[UG{self.id}] Válvula Borboleta Falha Abertura")
        self.condicionadores.append(c.CondicionadorBase(self.l_valv_borb_falha_abertura, CONDIC_NORMALIZAR))


        # ## CONDICIONADORES INDISPONIBILIZAR
        # ENTRADAS DIGITAIS
        self.l_status_rv = lei.LeituraModbusCoil(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_STT_RV"], descricao=f"[UG{self.id}] RV Status")
        self.condicionadores.append(c.CondicionadorBase(self.l_status_rv, CONDIC_INDISPONIBILIZAR))

        self.l_falha_fechar_distrib = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_RV_FALHA_AO_FECHAR_DISTRIBUIDOR"], descricao=f"[UG{self.id}] Falha Fechamento Distribuidor")
        self.condicionadores.append(c.CondicionadorBase(self.l_falha_fechar_distrib, CONDIC_INDISPONIBILIZAR))

        self.l_contro_trip_dif_grade = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_CONTROLE_TRIP_DIFERENCIAL_GRADE"], descricao=f"[UG{self.id}] Contorle Trip Diferencial Grade")
        self.condicionadores.append(c.CondicionadorBase(self.l_contro_trip_dif_grade, CONDIC_INDISPONIBILIZAR))

        self.l_resis_aquec_gera_falha_deslig = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_RESISTENCIA_AQUEC_GERADOR_FALHA_DESLIGAR"], descricao=f"[UG{self.id}] Resistência Aquecedor Gerador Falha Desligar")
        self.condicionadores.append(c.CondicionadorBase(self.l_resis_aquec_gera_falha_deslig, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_partir = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_RV_FALHA_AO_PARTIR"], descricao=f"[UG{self.id}] RV Falha Partir")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_partir, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_desab = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_RV_FALHA_AO_DESABILITAR"], descricao=f"[UG{self.id}] RV Falha Desabilitar")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_desab, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_parar_maqu = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_RV_FALHA_AO_PARAR_MAQUINA"], descricao=f"[UG{self.id}] RV Falha Parar Máquina")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_parar_maqu, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_partir = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_RT_FALHA_AO_PARTIR"], descricao=f"[UG{self.id}] RT Falha Partir")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_partir, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_desab = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_RT_FALHA_AO_DESABILITAR"], descricao=f"[UG{self.id}] RT Falha Desabilitar")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_desab, CONDIC_INDISPONIBILIZAR))

        self.l_valv_borb_falha_fechar = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_BORBOLETA_FALHA_FECHAR"], descricao=f"[UG{self.id}] Válvula Borboleta Falha Fechar")
        self.condicionadores.append(c.CondicionadorBase(self.l_valv_borb_falha_fechar, CONDIC_INDISPONIBILIZAR))

        self.l_valv_borb_dicrep_senso = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_BORBOLETA_DISCREPANCIA_SENSORES"], descricao=f"[UG{self.id}] Válvula Borboleta Discrepância Sensores")
        self.condicionadores.append(c.CondicionadorBase(self.l_valv_borb_dicrep_senso, CONDIC_INDISPONIBILIZAR))

        self.l_vavl_bypass_discrep_senso = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_BYPASS_DISCREPANCIA_SENSORES"], descricao=f"[UG{self.id}] Válvula Bypass Discrepância Sensores")
        ...
        self.l_uhrv_oleo_nv_muito_baixo = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_PRTVA_UHRV_OLEO_NIVEL_MUITO_BAIXO"], descricao=f"[UG{self.id}] UHRV Óleo Nível Muito Baixo")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhrv_oleo_nv_muito_baixo, CONDIC_INDISPONIBILIZAR))

        self.l_uhrv_filtro_oleo_sujo = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_PRTVA_UHRV_FILTRO_OLEO_SUJO"], descricao=f"[UG{self.id}] UHRV Filtro Óleo Sujo")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhrv_filtro_oleo_sujo, CONDIC_INDISPONIBILIZAR))

        self.l_urhv_press_crit = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_PRTVA_UHRV_PRESSAO_CRITICA"], descricao=f"[UG{self.id}] UHRV Pressão Crítica")
        self.condicionadores.append(c.CondicionadorBase(self.l_urhv_press_crit, CONDIC_INDISPONIBILIZAR))

        self.l_uhrv_oleo_nv_muito_alto = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_PRTVA_UHLM_OLEO_NIVEL_MUITO_ALTO"], descricao=f"[UG{self.id}] UHRV Óleo Nível Muito Alto")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhrv_oleo_nv_muito_alto, CONDIC_INDISPONIBILIZAR))

        self.l_uhlm_oleo_nv_muito_baixo = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_PRTVA_UHLM_OLEO_NIVEL_MUITO_BAIXO"], descricao=f"[UG{self.id}] UHLM Óleo Nível Muito Baixo")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhlm_oleo_nv_muito_baixo, CONDIC_INDISPONIBILIZAR))

        # TODO -> Verificar
        self.l_uhlm_press_linha_lubrifi = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_PRTVA_UHLM_PRESSAO_LINHA_LUBRIFICACAO"], descricao=f"[UG{self.id}] UHLM Pressão Linha Lubrificação")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhlm_press_linha_lubrifi, CONDIC_NORMALIZAR))

        # TODO n-> Verificar
        self.l_uhlm_fluxo_troc_calor = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_PRTVA_UHLM_FLUXO_TROCADOR_CALOR"], descricao=f"[UG{self.id}] UHLM Fluxo Trocador Calor")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhlm_fluxo_troc_calor, CONDIC_NORMALIZAR))

        # TODO -> Verificar
        self.l_qbag_escova_polo_pos_desgas = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_PRTVA_QBAG_ESCOVA_POLO_POSITIVO_DESGASTADA"], descricao=f"[UG{self.id}] QBAG Escova Polo Positivo Desgastada")
        self.condicionadores.append(c.CondicionadorBase(self.l_qbag_escova_polo_pos_desgas, CONDIC_INDISPONIBILIZAR))

        self.l_qbag_escova_polo_neg_desgas = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_PRTVA_QBAG_ESCOVA_POLO_NEGATIVO_DESGASTADA"], descricao=f"[UG{self.id}] QBAG Escova Polo Negativo Desgastada")
        self.condicionadores.append(c.CondicionadorBase(self.l_qbag_escova_polo_neg_desgas, CONDIC_INDISPONIBILIZAR))

        self.l_rele_prot_gerad_50bf = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_PRTVA_RELE_PROT_GERADOR_50BF"], descricao=f"[UG{self.id}] Relé Proteção Gerador 50BF")
        self.condicionadores.append(c.CondicionadorBase(self.l_rele_prot_gerad_50bf, CONDIC_INDISPONIBILIZAR))

        self.l_disj_tps_protecao = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_PRTVA_DISJUNTOR_TPS_PROTECAO"], descricao=f"[UG{self.id}] Disjuntor TPS Proteção")
        self.condicionadores.append(c.CondicionadorBase(self.l_disj_tps_protecao, CONDIC_INDISPONIBILIZAR))

        # TODO -> Verificar
        self.l_uhrv_pressao_freio = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_PRTVA_UHRV_PRESSAO_FREIO"], descricao=f"[UG{self.id}] UHRV Pressão Freio")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhrv_pressao_freio, CONDIC_NORMALIZAR))

        self.l_rv_falha_hab = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_RV_FALHA_AO_HABILITAR"], descricao=f"[UG{self.id}] RV Falha Habilitar")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_hab, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_hab = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_RT_FALHA_AO_HABILITAR"], descricao=f"[UG{self.id}] RT Falha Habilitar")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_hab, CONDIC_INDISPONIBILIZAR))

        # UHRV
        # TODO -> Verificar
        self.l_uhrv_bomba_1_indisp = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_UHRV_BOMBA_1_INDISPONIVEL"], descricao=f"[UG{self.id}] UHRV Bomba 1 Indisponível") # TODO -> Verificar invertido
        self.condicionadores.append(c.CondicionadorBase(self.l_uhrv_bomba_1_indisp, CONDIC_INDISPONIBILIZAR))

        self.l_uhrv_bomba_1_falha_ligar = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_UHRV_BOMBA_1_FALHA_AO_LIGAR"], descricao=f"[UG{self.id}] UHRV Falha Ligar")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhrv_bomba_1_falha_ligar, CONDIC_INDISPONIBILIZAR))

        self.l_uhrv_filtro_oleo_sujo = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_UHRV_FILTRO_OLEO_SUJO"], descricao=f"[UG{self.id}] UHRV Filtro Óleo Sujo")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhrv_filtro_oleo_sujo, CONDIC_INDISPONIBILIZAR))

        # UHLM
        self.l_uhlm_bomba_1_falha_ligar = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_UHLM_BOMBA_1_FALHA_LIGAR"], descricao=f"[UG{self.id}] UHLM Bomba 1 Falha Ligar")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhlm_bomba_1_falha_ligar, CONDIC_INDISPONIBILIZAR))

        self.l_uhlm_bomba_1_falha_deslig = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_UHLM_BOMBA_1_FALHA_DESLIGAR"], descricao=f"[UG{self.id}] UHLM Bomba 1 Falha Desligar")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhlm_bomba_1_falha_deslig, CONDIC_INDISPONIBILIZAR))

        self.l_ulhm_falha_pressos = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_UHLM_FALHA_PRESSOSTATO"], descricao=f"[UG{self.id}] UHLM Falha Pressostato")
        self.condicionadores.append(c.CondicionadorBase(self.l_ulhm_falha_pressos, CONDIC_INDISPONIBILIZAR))

        self.l_uhlm_bomba_1_indisp = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_UHLM_BOMBA_1_INDISPONIVEL"], descricao=f"[UG{self.id}] UHLM Bomba 1 Indisponível")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhlm_bomba_1_indisp, CONDIC_INDISPONIBILIZAR))

        self.l_uhlm_bomba_1_falha_pressurizar = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_UHLM_BOMBA_1_FALHA_PRESSURIZAR"], descricao=f"[UG{self.id}] UHLM Bomba 1 Falha Pressurizar")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhlm_bomba_1_falha_pressurizar, CONDIC_INDISPONIBILIZAR))

        self.l_ulhm_filtro_sujo = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_UHLM_FILTRO_SUJO"], descricao=f"[UG{self.id}] UHLM Filtro Sujo")
        self.condicionadores.append(c.CondicionadorBase(self.l_ulhm_filtro_sujo, CONDIC_INDISPONIBILIZAR))

        # RV
        self.l_rv_girando_gir_indev = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RV_FALHA_1_GIRANDO_SEM_REG_GIRO_INDEV"], descricao=f"[UG{self.id}] Falha RV Girando Sem Registro de Giro Indevido")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_girando_gir_indev, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_1_sobrefreq_inst = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RV_FALHA_1_SOBREFREQ_INSTANT"], descricao=f"[UG{self.id}] RV Falha 1 Sobrefrequência Instantânea")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_1_sobrefreq_inst, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_1_sobrefreq_tempor = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RV_FALHA_1_SOBREFREQ_TEMPOR"], descricao=f"[UG{self.id}] RV Falha 1 Sobrefrequência Temporizada")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_1_sobrefreq_tempor, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_1_subfreq_tempor = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RV_FALHA_1_SUBFREQ_TEMPORIZADA"], descricao=f"[UG{self.id}] RV Falha 1 Subfrequência Temporizada")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_1_subfreq_tempor, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_1_leit_pos_distrib = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RV_FALHA_1_LEIT_POS_DISTRIBUIDOR"], descricao=f"[UG{self.id}] RV Falha 1 Leitura Posição Distribuidor")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_1_leit_pos_distrib, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_1_leit_pot_ativa = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RV_FALHA_1_LEIT_POTENCIA_ATIVA"], descricao=f"[UG{self.id}] RV Falha 1 Leitura Potência Ativa")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_1_leit_pot_ativa, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_1_leit_refer_pot = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RV_FALHA_1_LEIT_REFERENCIA_POTENCIA"], descricao=f"[UG{self.id}] RV Falha 1 Leitura Referência Potência")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_1_leit_refer_pot, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_1_nv_montante_muito_baixo = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RV_FALHA_1_NV_MONTANTE_MUITO_BAIXO"], descricao=f"[UG{self.id}] RV Falha 1 Nível Montante Muito Baixo")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_1_nv_montante_muito_baixo, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_1_control_pos_distribu = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RV_FALHA_1_CONTROLE_POS_DISTRIBUIDOR"], descricao=f"[UG{self.id}] RV Falha 1 Controle Posição Distribuidor")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_1_control_pos_distribu, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_1_ruido_med_veloc_princi = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RV_FALHA_1_RUIDO_MED_VELOC_PRINCIPAL"], descricao=f"[UG{self.id}] RV Falha 1 Ruído Medição Velocidade Principal")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_1_ruido_med_veloc_princi, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_1_ruido_med_veloc_retag = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RV_FALHA_1_RUIDO_MED_VELOC_RETAGUARDA"], descricao=f"[UG{self.id}] RV Falha 1 Ruído Medição Velocidade Retaguarda")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_1_ruido_med_veloc_retag, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_2_perda_med_veloc_retag = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RV_FALHA_2_PERDA_MED_VELOC_RETAGUARDA"], descricao=f"[UG{self.id}] RV Falha 2 Perda Medição Velocidade Retaguarda")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_2_perda_med_veloc_retag, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_2_tempo_excess_partida = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RV_FALHA_2_TEMPO_EXCESSIVO_PARTIDA"], descricao=f"[UG{self.id}] RV Falha 2 Tempo Excessivo Partida")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_2_tempo_excess_partida, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_2_tempo_excess_parada = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RV_FALHA_2_TEMPO_EXCESSIVO_PARADA"], descricao=f"[UG{self.id}] RV Falha 2 Tempo Excessivo Parada")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_2_tempo_excess_parada, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_2_dif_med_velo_princ_retag = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RV_FALHA_2_DIF_MED_VELO_PRINCIPAL_RETAGUARDA"], descricao=f"[UG{self.id}] RV Falha 2 Diferença Medição Velocidade Principal Retaguarda")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_2_dif_med_velo_princ_retag, CONDIC_INDISPONIBILIZAR))

        self.l_rv_falha_1_perda_med_velo_princ = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RV_FALHA_1_PERDA_MED_VELOC_PRINCIPAL"], descricao=f"[UG{self.id}] RV Falha 1 Perda Medição Velocidade Principal")
        self.condicionadores.append(c.CondicionadorBase(self.l_rv_falha_1_perda_med_velo_princ, CONDIC_INDISPONIBILIZAR))

        # RT
        self.l_rt_crowbar_inativo = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ED_CROWBAR_INATIVO"], descricao=f"[UG{self.id}] RT Crowbar Inativo")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_crowbar_inativo, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_sobretensao = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_1_SOBRETENSAO"], descricao=f"[UG{self.id}] RT Alarmes 1 Sobretensão")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_sobretensao, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_subtensao = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_1_SUBTENSAO"], descricao=f"[UG{self.id}] RT Alarmes 1 Subtensão")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_subtensao, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_sobrefrequencia = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_1_SOBREFREQUENCIA"], descricao=f"[UG{self.id}] RT Alarmes 1 Sobrefrequência")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_sobrefrequencia, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_subfrequencia = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_1_SUBFREQUENCIA"], descricao=f"[UG{self.id}] RT Alarmes 1 Subfrequência")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_subfrequencia, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_lim_sup_pot_reativa = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_1_LIMITE_SUP_POT_REATIVA"], descricao=f"[UG{self.id}] RT Alarmes 1 Limite Superior Potência Reativa")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_lim_sup_pot_reativa, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_lim_inf_pot_reativa = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_1_LIMITE_INF_POT_REATIVA"], descricao=f"[UG{self.id}] RT Alarmes 1 Limite Inferior Potência Reativa")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_lim_inf_pot_reativa, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_lim_sup_fator_pot = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_1_LIMITE_SUP_FATOR_POTENCIA"], descricao=f"[UG{self.id}] RT Alarmes 1 Limite Superior Fator Potência")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_lim_sup_fator_pot, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_lim_inf_fator_pot = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_1_LIMITE_INF_FATOR_POTENCIA"], descricao=f"[UG{self.id}] RT Alarmes 1 Limite Inferior Fator Potência")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_lim_inf_fator_pot, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_variacao_tensao = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_1_VARIACAO_TENSAO"], descricao=f"[UG{self.id}] RT Alarmes 1 Variação de Tensão")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_variacao_tensao, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_pot_ativa_reversa = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_1_POTENCIA_ATIVA_REVERSA"], descricao=f"[UG{self.id}] RT Alarmes 1 Potência Ativa Reversa")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_pot_ativa_reversa, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_sobrecorr_term = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_1_SOBRECORRENTE_TERMINAL"], descricao=f"[UG{self.id}] RT Alarmes 1 Sobrecorrente Terminal")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_sobrecorr_term, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_lim_sup_corr_excitacao = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_1_LIMITE_SUP_CORRENTE_EXCITACAO"], descricao=f"[UG{self.id}] RT Alarmes 1 Limite Superior Corrente Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_lim_sup_corr_excitacao, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_lim_inf_corr_exci = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_1_LIMITE_INF_CORRENTE_EXCITACAO"], descricao=f"[UG{self.id}] RT Alarmes 1 Limite Inferior Corrente Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_lim_inf_corr_exci, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_temp_muito_alta_rotor = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_1_TEMP_MUITO_ALTA_ROTOR"], descricao=f"[UG{self.id}] RT Alarmes 1 Temperatura Muito Alta Rotor")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_temp_muito_alta_rotor, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_pres_tens_term_aus_corr_exci = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_1_PRES_TENS_TERM_AUSEN_CORR_EXCI"], descricao=f"[UG{self.id}] RT Alarmes 1 Presença Tensão Terminal Ausente Corrente Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_pres_tens_term_aus_corr_exci, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_1_pres_corr_exci_aus_tens_term = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_1_PRES_CORR_EXCI_AUSEN_TENS_TERM"], descricao=f"[UG{self.id}] RT Alarmes 1 Presença Corrente Excitação Ausente Tensão Terminal")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_1_pres_corr_exci_aus_tens_term, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_2_falha_contro_corr_exci = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_2_FALHA_CONTROLE_CORRENTE_EXCI"], descricao=f"[UG{self.id}] RT Alarmes 2 Falha Controle Corrente Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_2_falha_contro_corr_exci, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_2_falha_contro_tens_term = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_2_FALHA_CONTROLE_TENSAO_TERM"], descricao=f"[UG{self.id}] RT Alarmes 2 Falha Controle Tensão Terminal")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_2_falha_contro_tens_term, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_2_crowbar_atuado_regul_hab = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_2_CROWBAR_ATUADO_REGUL_HABIL"], descricao=f"[UG{self.id}] RT Alarme 2 Crowbar Atuado Regulador Habilitado")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_2_crowbar_atuado_regul_hab, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_2_falha_hab_drive_excit = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_2_FALHA_HABIL_DRIVE_EXCI"], descricao=f"[UG{self.id}] RT Alarme 2 Falha Habilitar Drive Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_2_falha_hab_drive_excit, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_2_falha_fechar_contator_campo = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_2_FALHA_FECHAR_CONTATOR_CAMPO"], descricao=f"[UG{self.id}] RT Alarme 2 Falha Fechar Contator Campo")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_2_falha_fechar_contator_campo, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_2_falha_corr_exci_pre_exci_ativa = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_2_FALHA_CORR_EXCI_PRE_EXCI_ATIVA"], descricao=f"[UG{self.id}] RT Alarme 2 Falha Corrente Excitação Pré Excitação Ativa")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_2_falha_corr_exci_pre_exci_ativa, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_2_perda_med_pot_reat = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_2_PERDA_MEDICAO_POTENCIA_REATIVA"], descricao=f"[UG{self.id}] RT Alarme 2 Perda Medição Potência Reativa")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_2_perda_med_pot_reat, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_2_perda_med_tens_term = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_2_PERDA_MEDICAO_TENSAO_TERMINAL"], descricao=f"[UG{self.id}] RT Alarme 2 Perda Medição Tensão Terminal")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_2_perda_med_tens_term, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_2_perda_med_corr_exci = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_2_PERDA_MEDICAO_CORRENTE_EXCI"], descricao=f"[UG{self.id}] RT Alarme 2 Perda Medição Corrente Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_2_perda_med_corr_exci, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_ruido_intrumen_reat = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_2_RUIDO_INSTRUMEN_REATIVO"], descricao=f"[UG{self.id}] RT Alarme 2 Ruído Leitura Intrumentador Reativo")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_ruido_intrumen_reat, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_ruido_intrumen_tensao = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_2_RUIDO_INSTRUMEN_TENSAO"], descricao=f"[UG{self.id}] RT Alarme 2 Ruído Leitura Intrumentador Tensão")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_ruido_intrumen_tensao, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_ruido_intrumen_exci_princ = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_2_RUIDO_INSTRUMEN_EXCI_PRINCI"], descricao=f"[UG{self.id}] RT Alarme 2 Ruído Leitura Intrumentador Excitação Principal")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_ruido_intrumen_exci_princ, CONDIC_INDISPONIBILIZAR))

        self.l_rt_alar_ruido_intrumen_exci_retag = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ALARMES_2_RUIDO_INSTRUMEN_EXCI_RETAG"], descricao=f"[UG{self.id}] RT Alarme 2 Ruído Leitura Intrumentador Excitação Retaguarda")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_alar_ruido_intrumen_exci_retag, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_sobretensao = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_1_SOBRETENSAO"], descricao=f"[UG{self.id}] RT Falha 1 Sobretensão")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_sobretensao, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_subtensao = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_1_SUBTENSAO"], descricao=f"[UG{self.id}] RT Falha 1 Subtensão")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_subtensao, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_sobrefreq = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_1_SOBREFREQUENCIA"], descricao=f"[UG{self.id}] RT Falha 1 Sobrefrequência")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_sobrefreq, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_subfreq = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_1_SUBFREQUENCIA"], descricao=f"[UG{self.id}] RT Falha 1 Subfrequência")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_subfreq, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_lim_sup_pot_reat = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_1_LIMITE_SUP_POT_REATIVA"], descricao=f"[UG{self.id}] RT Falha 1 Limite Superior Potência Reativa")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_lim_sup_pot_reat, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_lim_inf_pot_reat = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_1_LIMITE_INF_POT_REATIVA"], descricao=f"[UG{self.id}] RT Falha 1 Limite Inferior Potência Reativa")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_lim_inf_pot_reat, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_lim_sup_fator_pot = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_1_LIMITE_SUP_FATOR_POT"], descricao=f"[UG{self.id}] RT Falha 1 Limite Superior Fator Potência")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_lim_sup_fator_pot, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_lim_inf_fator_pot = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_1_LIMITE_INF_FATOR_POT"], descricao=f"[UG{self.id}] RT Falha 1 Limite Inferior Fator Potência")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_lim_inf_fator_pot, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_sobretensao_inst = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_1_SOBRETENSAO_INST"], descricao=f"[UG{self.id}] RT Falha 1 Sobretensão Instantânea")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_sobretensao_inst, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_variacao_tensao = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_1_VARIACAO_TENSAO"], descricao=f"[UG{self.id}] RT Falha 1 Variação Tensão")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_variacao_tensao, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_pot_ativ_reversa = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_1_POT_ATIVA_REVERSA"], descricao=f"[UG{self.id}] RT Falha 1 Potência Ativa Reversa")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_pot_ativ_reversa, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_sobrecorr_term = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_1_SOBRECORRENTE_TERMINAL"], descricao=f"[UG{self.id}] RT Falha 1 Sobrecorrente Terminal")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_sobrecorr_term, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_lim_sup_corr_exci = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_1_LIMITE_SUP_CORRENTE_EXCITACAO"], descricao=f"[UG{self.id}] RT Falha 1 Limite Superior Corrente Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_lim_sup_corr_exci, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_lim_inf_corr_exci = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_1_LIMITE_INF_CORRENTE_EXCITACAO"], descricao=f"[UG{self.id}] RT Falha 1 Limite Inferior Corrente Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_lim_inf_corr_exci, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_lim_sup_tensao_exci = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_1_LIMITE_SUP_TENSAO_EXCITACAO"], descricao=f"[UG{self.id}] RT Falha 1 Limite Superior Tensão Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_lim_sup_tensao_exci, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_1_lim_inf_tensao_exci = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_1_LIMITE_INF_TENSAO_EXCITACAO"], descricao=f"[UG{self.id}] RT Falha 1 Limite Inferior Tensão Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_1_lim_inf_tensao_exci, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_2_temp_muito_alta_rotor = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_2_TEMP_MUITO_ALTA_ROTOR"], descricao=f"[UG{self.id}] RT Falha 2 Temperatura Muito Alta Rotor")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_2_temp_muito_alta_rotor, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_2_pres_tens_term_aus_corr_exci = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_2_PRES_TENS_TERM_AUSEN_CORR_EXCI"], descricao=f"[UG{self.id}] RT Falha 2 Presença Tensão Terminal Ausente Corrente Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_2_pres_tens_term_aus_corr_exci, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_2_pres_corr_exci_aus_tens_term = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_2_PRES_CORR_EXCI_AUSEN_TENS_TERM"], descricao=f"[UG{self.id}] RT Falha 2 Presença Corrente Excitação Ausente Tensão Terminal")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_2_pres_corr_exci_aus_tens_term, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_2_control_corr_exci = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_2_CONTROLE_CORR_EXCI"], descricao=f"[UG{self.id}] RT Falha 2 Controle Corrente Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_2_control_corr_exci, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_2_tensao_term = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_2_TENSAO_TERMINAL"], descricao=f"[UG{self.id}] RT Falha 2 Tensão Terminal")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_2_tensao_term, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_2_crowbar_atuado_regu_hab = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_2_CROWBAR_ATUADO_REGULADOR_HABI"], descricao=f"[UG{self.id}] RT Falha 2 Crowbar Atuado Regulador Habilitado")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_2_crowbar_atuado_regu_hab, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_2_hab_drive_exci = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_2_HABI_DRIVE_EXCITACAO"], descricao=f"[UG{self.id}] RT Falha 2 Habilitar Drive Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_2_hab_drive_exci, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_2_fechar_contator_campo = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_2_FECHAR_CONTATOR_CAMPO"], descricao=f"[UG{self.id}] RT Falha 2 Fechar Contator Campo")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_2_fechar_contator_campo, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_2_corr_exci_pre_exci_ativa = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_2_CORR_EXCITA_PRE_EXCXITA_ATIVA"], descricao=f"[UG{self.id}] RT Falha 2 Corrente Excitação Pré Excitada Ativa")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_2_corr_exci_pre_exci_ativa, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_2_excess_pre_exci = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_2_EXCESSIVO_PRE_EXCITACAO"], descricao=f"[UG{self.id}] RT Falha 2 Excessivo Pré Excitação")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_2_excess_pre_exci, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_2_excess_parada = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_2_EXCESSIVO_PARADA"], descricao=f"[UG{self.id}] RT Falha 2 Excessivo Parada")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_2_excess_parada, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_3_perda_med_pot_reativa = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_3_PERDA_MED_POT_REATIVA"], descricao=f"[UG{self.id}] RT Falha 3 Perda Medição Potência Reativa")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_3_perda_med_pot_reativa, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_3_perda_med_tensao_term = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_3_PERDA_MED_TENSAO_TERM"], descricao=f"[UG{self.id}] RT Falha 3 Perda Medição Tensão Terminal")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_3_perda_med_tensao_term, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_3_perda_med_corr_exci_princ = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_3_PERDA_MED_CORR_EXCI_PRINCI"], descricao=f"[UG{self.id}] RT Falha 3 Perda Medição Corrente Excitação Principal")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_3_perda_med_corr_exci_princ, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_3_perda_med_corr_exci_retag = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_3_PERDA_MED_CORR_EXCI_RETAG"], descricao=f"[UG{self.id}] RT Falha 3 Perda Medição Corrente Excitação Retaguarda")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_3_perda_med_corr_exci_retag, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_3_ruido_leit_instrum_reativo = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_3_RUIDO_INSTRUM_REATIVO"], descricao=f"[UG{self.id}] RT Falha 3 Ruído Leitura Instrumentador Reativo")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_3_ruido_leit_instrum_reativo, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_3_ruido_leit_instrum_tensao = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_3_RUIDO_INSTRUM_TENSAO"], descricao=f"[UG{self.id}] RT Falha 3 Ruído Leitura Instrumentador Tensão")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_3_ruido_leit_instrum_tensao, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_3_ruido_leit_instrum_principal = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_3_RUIDO_INSTRUM_EXCI_PRINCI"], descricao=f"[UG{self.id}] RT Falha 3 Ruído Leitura Instrumentador Principal")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_3_ruido_leit_instrum_principal, CONDIC_INDISPONIBILIZAR))

        self.l_rt_falha_3_ruido_leit_instrum_retag = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_FALHAS_3_RUIDO_INSTRUM_EXCI_RETAG"], descricao=f"[UG{self.id}] RT Falha 3 Ruído Leitura Instrumentador Retaguarda")
        self.condicionadores.append(c.CondicionadorBase(self.l_rt_falha_3_ruido_leit_instrum_retag, CONDIC_INDISPONIBILIZAR))


        # ENTRADAS ANALÓGICAS
        self.l_tristor_temp_muito_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_TRISTORES_TEMP_MUITO_ALTA"], descricao=f"[UG{self.id}] Tristores Temperatura Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_tristor_temp_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_crowbar_temp_muito_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_CROWBAR_TEMP_MUITO_ALTA"], descricao=f"[UG{self.id}] Crowbar Temperatura Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_crowbar_temp_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_trafo_exci_temp_muito_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_TRAFO_EXCITACAO_TEMP_MUITO_ALTA"], descricao=f"[UG{self.id}] Transformador Excitação Temperatura Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_trafo_exci_temp_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_uhrv_temp_oleo_muito_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_UHRV_OLEO_TEMP_MUITO_ALTA"], descricao=f"[UG{self.id}] UHRV Óleo Temperatura Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhrv_temp_oleo_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_gera_fase_a_temp_muito_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_GERADOR_FASE_A_TEMP_MUITO_ALTA"], descricao=f"[UG{self.id}] Gerador Fase A Temperatura Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_gera_fase_a_temp_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_gera_fase_b_temp_muito_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_GERADOR_FASE_B_TEMP_MUITO_ALTA"], descricao=f"[UG{self.id}] Gerador Fase B Temperatura Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_gera_fase_b_temp_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_gera_fase_c_temp_muito_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_GERADOR_FASE_C_TEMP_MUITO_ALTA"], descricao=f"[UG{self.id}] Gerador Fase C Temperatura Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_gera_fase_c_temp_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_gera_nucleo_1_temp_muito_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_GERADOR_NUCLEO_1_TEMP_MUITO_ALTA"], descricao=f"[UG{self.id}] Gerador Núcleo 1 Temperatura Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_gera_nucleo_1_temp_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_gera_nucleo_2_temp_muito_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_GERADOR_NUCLEO_2_TEMP_MUITO_ALTA"], descricao=f"[UG{self.id}] Gerador Núcleo 2 Temperatura Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_gera_nucleo_2_temp_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_gera_nucleo_3_temp_muito_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_GERADOR_NUCLEO_3_TEMP_MUITO_ALTA"], descricao=f"[UG{self.id}] Gerador Núcleo 3 Temperatura Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_gera_nucleo_3_temp_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_mancal_guia_casq_temp_muito_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_MANCAL_GUIA_CASQUILHO_TEMP_MUITO_ALTA"], descricao=f"[UG{self.id}] Mancal Guia Casquilho Temperatura Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_mancal_guia_casq_temp_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_mancal_comb_casq_temp_muito_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_MANCAL_COMBINADO_CASQUILHO_TEMP_MUITO_ALTA"], descricao=f"[UG{self.id}] Mancal Combinado Casquilho Temperatura Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_mancal_comb_casq_temp_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_mancal_comb_esc_temp_muito_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_MANCAL_COMBINADO_ESCORA_TEMP_MUITO_ALTA"], descricao=f"[UG{self.id}] Mancal Combinado Escora Temperatura Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_mancal_comb_esc_temp_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_uhrv_press_oleo_falha_leitura = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_UHRV_PRESSAO_OLEO_FALHA_LEITURA"], descricao=f"[UG{self.id}] UHRV Pressão Óleo Falha Leitura")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhrv_press_oleo_falha_leitura, CONDIC_INDISPONIBILIZAR))

        self.l_uhrv_press_oleo_muito_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_UHRV_PRESSAO_OLEO_MUITO_ALTO"], descricao=f"[UG{self.id}] UHRV Pressão Óleo Muito Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhrv_press_oleo_muito_alta, CONDIC_INDISPONIBILIZAR))

        self.l_uhrv_press_oleo_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_UHRV_PRESSAO_OLEO_ALTO"], descricao=f"[UG{self.id}] UHRV Pressão Óleo Alta")
        self.condicionadores.append(c.CondicionadorBase(self.l_uhrv_press_oleo_alta, CONDIC_INDISPONIBILIZAR))


        # ## WHATSAPP + VOIP
        self.l_cmd_uhrv_modo_manuten = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_CD_CMD_UHRV_MODO_MANUTENCAO"], descricao=f"[UG{self.id}] UHRV Comando Modo Manutenção")
        self.l_cmd_uhlm_modo_manuten = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_CD_CMD_UHLM_MODO_MANUTENCAO"], descricao=f"[UG{self.id}] UHLM Comando Modo Manutenção")
        self.l_resis_quec_gerador_falha_ligar = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_RESISTENCIA_AQUEC_GERADOR_FALHA_LIGAR"], descricao=f"[UG{self.id}] ")

        self.l_falha_leit_temp_tristores = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_TRISTORES_TEMP_FALHA_LEITURA"], descricao=f"[UG{self.id}] Tristores Temperatura Falha Leitura")
        self.l_falha_leit_temp_crowbar = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_CROWBAR_TEMP_FALHA_LEITURA"], descricao=f"[UG{self.id}] Crowbar Temperatura Falha Leitura")
        self.l_falha_leit_temp_uhrv_temp_oleo = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_UHRV_TEMP_OLEO_FALHA_LEITURA"], descricao=f"[UG{self.id}] UHRV Temperatura Óleo Falha Leitura")
        self.l_falha_leit_temp_trafo_exci = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_TRAFO_EXCITACAO_FALHA_LEITURA"], descricao=f"[UG{self.id}] Transformador Excitação Temperatura Falha Leitura")
        self.l_falha_leit_temp_gerad_fase_a = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_GERADOR_FASE_A_TEMP_FALHA_LEITURA"], descricao=f"[UG{self.id}] Gerador Fase A Temperatura Falha Leitura")
        self.l_falha_leit_temp_gerad_fase_b = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_GERADOR_FASE_B_TEMP_FALHA_LEITURA"], descricao=f"[UG{self.id}] Gerador Fase B Temperatura Falha Leitura")
        self.l_falha_leit_temp_gerad_fase_c = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_GERADOR_FASE_C_TEMP_FALHA_LEITURA"], descricao=f"[UG{self.id}] Gerador Fase C Temperatura Falha Leitura")
        self.l_falha_leit_temp_gerad_nucleo_1 = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_GERADOR_NUCLEO_1_TEMP_FALHA_LEITURA"], descricao=f"[UG{self.id}] Gerador Núcleo 1 Temperatura Falha Leitura")
        self.l_falha_leit_temp_gerad_nucleo_2 = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_GERADOR_NUCLEO_2_TEMP_FALHA_LEITURA"], descricao=f"[UG{self.id}] Gerador Núcleo 2 Temperatura Falha Leitura")
        self.l_falha_leit_temp_gerad_nucleo_3 = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_GERADOR_NUCLEO_3_TEMP_FALHA_LEITURA"], descricao=f"[UG{self.id}] Gerador Núcleo 3 Temperatura Falha Leitura")
        self.l_falha_leit_temp_mancal_guia_casq = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_MANCAL_GUIA_CASQUILHO_TEMP_FALHA_LEITURA"], descricao=f"[UG{self.id}] Mancal Guia Casquilho temperatura Falha Leitura")
        self.l_falha_leit_temp_mancal_comb_esc = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_MANCAL_COMBINADO_ESCORA_TEMP_FALHA_LEITURA"], descricao=f"[UG{self.id}] Mancal Combinado Escora Temperatura Falha Leitura")
        self.l_falha_leit_temp_mancal_comb_casq = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_MANCAL_COMBINADO_CASQUILHO_TEMP_FALHA_LEITURA"], descricao=f"[UG{self.id}] Mancal Combinado Casquilho Temperatura Falha Leitura")

        # ## WHATSAPP
        self.l_rv_alarme = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_PRTVA_RV_ALARME"], descricao=f"[UG{self.id}] RV Alarme")
        self.l_rt_alarme = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_PRTVA_RT_ALARME"], descricao=f"[UG{self.id}] RT Alarme")
        self.l_alarme_rele = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_SD_RELE_ALARME"], descricao=f"[UG{self.id}] Alarme Relé")
        self.l_rv_pot_nula = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_PRTVA_RV_POTENCIA_NULA"], descricao=f"[UG{self.id}] RV Potência Nula")
        self.l_uhrv_bomba_defeito = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_PRTVA_UHRV_BOMBA_DEFEITO"], descricao=f"[UG{self.id}] UHRV Bomba Defeito") # TODO -> Verificar invertido
        self.l_uhlm_bomba_defeito = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_PRTVA_UHLM_BOMBA_DEFEITO"], descricao=f"[UG{self.id}] UHLM Bomba Defeito")
        self.l_dispo_prot_surto = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_PRTVA_DISPOSITIVO_PROTECAO_SURTO"], descricao=f"[UG{self.id}] Dispositivo Proteção Surto")
        self.l_rt_selec_modo_controle_isol = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_RT_ED_SELEC_MODO_CONTROLE_ISOLADO"], descricao=f"[UG{self.id}] RT Selecionado Modo Controle Isolado")
        self.l_resis_aquec_gerad_defeito = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_PRTVA_RESISTENCIA_AQUEC_GERADOR_DEFEITO"], descricao=f"[UG{self.id}] Resistência Aquecimento Gerador Defeito")

        self.l_crowbar_temp_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_CROWBAR_TEMP_ALTA"], descricao=f"[UG{self.id}] Temperatura Alta")
        self.l_tristores_temp_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_TRISTORES_TEMP_ALTA"], descricao=f"[UG{self.id}] Temperatura Alta")
        self.l_uhrv_oleo_temp_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_UHRV_OLEO_TEMP_ALTA"], descricao=f"[UG{self.id}] Temperatura Alta")
        self.l_trafo_exci_temp_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_TRAFO_EXCITACAO_TEMP_ALTA"], descricao=f"[UG{self.id}] Temperatura Alta")
        self.l_gerad_fase_a_temp_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_GERADOR_FASE_A_TEMP_ALTA"], descricao=f"[UG{self.id}] Gerador Fase A Temperatura Alta")
        self.l_gerad_fase_b_temp_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_GERADOR_FASE_B_TEMP_ALTA"], descricao=f"[UG{self.id}] Gerador Fase B Temperatura Alta")
        self.l_gerad_fase_c_temp_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_GERADOR_FASE_C_TEMP_ALTA"], descricao=f"[UG{self.id}] Gerador Fase C Temperatura Alta")
        self.l_gerad_nucleo_1_temp_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_GERADOR_NUCLEO_1_TEMP_ALTA"], descricao=f"[UG{self.id}] Gerador Núcleo 1 Temperatura Alta")
        self.l_gerad_nucleo_2_temp_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_GERADOR_NUCLEO_2_TEMP_ALTA"], descricao=f"[UG{self.id}] Gerador Núcleo 2 Temperatura Alta")
        self.l_gerad_nucleo_3_temp_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_GERADOR_NUCLEO_3_TEMP_ALTA"], descricao=f"[UG{self.id}] Gerador Núcleo 3 Temperatura Alta")
        self.l_mancal_guia_casq_temp_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_MANCAL_GUIA_CASQUILHO_TEMP_ALTA"], descricao=f"[UG{self.id}] Mancal Guia Casquilho Temperatura Alta")
        self.l_mancal_comb_esc_temp_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_MANCAL_COMBINADO_ESCORA_TEMP_ALTA"], descricao=f"[UG{self.id}] Mancal Combinado Escora Temperatura Alta")
        self.l_mancal_comb_casq_temp_alta = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_MANCAL_COMBINADO_CASQUILHO_TEMP_ALTA"], descricao=f"[UG{self.id}] Mancal Combinado Casquilho Temperatura Alta")

        self.l_rv_modo_manut = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_RV_MODO_MANUTENCAO"], descricao=f"[UG{self.id}] RV Modo Manutenção")
        self.l_uhlm_unidade_manut = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_UHLM_UNIDADE_MANUTENCAO"], descricao=f"[UG{self.id}] UHLM Unidade Manutenção")
        self.l_sinal_nv_jusante_alto = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_SINAL_NIVEL_JUSANTE_ALTO"], descricao=f"[UG{self.id}] Nível Jusante Sinal Alto")
        self.l_uhrv_pressao_oleo_baixa = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_UHRV_PRESSAO_OLEO_BAIXA"], descricao=f"[UG{self.id}] UHRV Pressão Óleo Baixa")
        self.l_sinal_nv_jusante_baixo = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_SINAL_NIVEL_JUSANTE_BAIXA"], descricao=f"[UG{self.id}] Nível Jusante Sinal Baixo")
        self.l_falha_leit_nv_jusante = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_SINAL_NIVEL_JUSANTE_FALHA_LEITURA"], descricao=f"[UG{self.id}] Nível Jusante Falha Leitura")
        self.l_sinal_nv_jusante_muito_alto = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_SINAL_NIVEL_JUSANTE_MUITO_ALTO"], descricao=f"[UG{self.id}] Nível Jusante Sinal Muito Alto")
        self.l_uhrv_pressao_oleo_muito_baixa = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_UHRV_PRESSAO_OLEO_MUITO_BAIXA"], descricao=f"[UG{self.id}] UHRV Pressão Óleo Muito Baixa")
        self.l_alarme_contro_dif_grade = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_CONTROLE_ALARME_DIFERENCIAL_GRADE"], descricao=f"[UG{self.id}] Alarme Controle Diferencial Grade")
        self.l_sinal_nv_jusante_muito_baixo = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_EA_SINAL_NIVEL_JUSANTE_MUITO_BAIXA"], descricao=f"[UG{self.id}] Nível Jusante Sinal Muito Baixo")
        self.l_resis_aquec_gerad_indisp = lei.LeituraModbusBit(self.clp[f"UG{self.id}"], REG["CONDIC_UG"][f"UG{self.id}_ED_RESISTENCIA_AQUEC_GERADOR_INDISPONIVEL"], descricao=f"[UG{self.id}] Resistência Aquecimento Gerador Indisponível")
