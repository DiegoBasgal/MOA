...
class ModoManual(State):
    ...
    def run(self) -> "object":
        ...
        self.usn.ler_valores()
        logger.debug(f"[USN] Leitura de Nível:                   {tda.TomadaAgua.l_nivel_montante.valor:0.3f}")
        logger.debug(f"[USN] Potência no medidor:                {se.Subestacao.l_potencia_medidor:0.3f}")
        logger.debug("")

        for ug in self.usn.ugs:
            logger.debug(f"[UG{ug.id}] Unidade:                            \"{UG_SM_STR_DCT[ug.estado]}\"")
        ...
        self.usn.controle_ie = (self.usn.ug1.potencia + self.usn.ug2.potencia + self.usn.ug3.potencia) / self.usn.cfg["pot_alvo_usina"]
