...
class Voip:
    ...
    def acionar_chamada(cls) -> "None":
        ...
        if cls.cfg["voz_habilitado"]:
            ...
            if vd.voip["WATCHDOG"][0]:
                for contato in cls.lista_padrao:
                    logger.info(f"[VOIP] Disparando torpedo de voz para: {contato[0]} ({contato[1]})")
                    data = {
                        "caller": f"{cls.cfg['caller_voip']}",
                        "called": f"{contato[1]}",
                        "audios": [{"audio": f"{vd.voip['WATCHDOG'][1]}", "positionAudio": 1,}],
                        "dtmfs": [],
                    }
                    cls.codificar_dados(data, headers)
                vd.voip["WATCHDOG"][0] = False

            if vd.voip["EMERGENCIA"][0]:
                for contato in lista_contatos:
                    logger.info(f"[VOIP] Disparando torpedo de voz para: {contato[0]} ({contato[1]})")
                    data = {
                        "caller": f"{cls.cfg['caller_voip']}",
                        "called": f"{contato[1]}",
                        "audios": [{"audio": f"{vd.voip['EMERGENCIA'][1]}", "positionAudio": 1,}],
                        "dtmfs": [],
                    }
                    cls.codificar_dados(data, headers)
                vd.voip["EMERGENCIA"][0] = False

            else:
                todos = []
                for _, vl in vd.voip.items():
                    if vl[0] and vl[1] == 0:
                        todos.append(vl[2])
                        vl[1] = 1
