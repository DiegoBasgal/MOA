...
def comandos_manual_view(request, *args, **kwargs):
   ...
   client = ModbusClient(host='0.0.0.0', port=502, timeout=0.5, unit_id=1)
   client_sa = ModbusClient("0.0.0.0", 502, unit_id=1, timeout=0.5)
   ...
   if client.open():
      ...
      if regs is None or regs[0] < 2000:
         context["ug1_state", "ug1_etapa",] = 99
      else:
...
def comando_dj52l(request, *args, **kwargs):
   global flag_dj
   usina = ParametrosUsina.objects.get(id=1)
   client = ModbusClient("0.0.0.0", 502, unit_id=1, timeout=0.5)
   context = {"usina": usina}
   client.open()
